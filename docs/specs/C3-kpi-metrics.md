# C3 — KPI / Business Metrics

> **Öncelik:** P1 · **Faz:** 2 · **Kapsam:** metrik kataloğu + alarm

## 1. Amaç & Kullanıcı Hikâyeleri

KPI tanımı → hesap kodu (SQL/Python) → periyodik değer çekimi → trend kartı + anomali alarmı + alarm kural editörü.

**Kabul senaryoları:**

1. Yeni KPI tanımı: hesap kodu (SQL/Python) + periyod.
2. KPI kartı: güncel değer, sparkline, hedef çizgisi, anomali uyarısı.
3. Alarm kuralı: eşik (mutlak/relative/anomali) editörü.

## 2. Backend Tasarımı

**Agent/Servis:** `ai_data_science_team/agents/c3_agent.py` — `langgraph` react-agent; deterministic çekirdek aynı paketin `tools/` altında.
...
**Veri modeli:**
- `c3_runs (run_id, status, params_json, result_json, created_at)`
- `c3_artifacts (artifact_ref, run_id, kind, payload)`
...
**Konum:** KPI Board (kanban grid) + metrik detay drawer + alarm kural editörü
...
- İlgili node tipi `metric.define` platform-api-app'te handler'a bağlı.
