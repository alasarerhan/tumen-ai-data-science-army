...
tool layer (``ai_data_science_team.tools.robustness``) with
LangChain ``@tool`` decorators and exposes the standard
``make_robustness_agent`` factory + ``F5Agent`` OO
...
from ai_data_science_team.tools.robustness import (  # noqa: E402, F401
    add_gaussian_noise,
    default_scenarios,
    evaluate_robustness,
    mask_features,
...
def add_gaussian_noise_wrapped(X: np.ndarray) -> Tuple[str, dict]:
...
@tool(response_format="content_and_artifact")
def mask_features_wrapped(X: np.ndarray, mask_rate: float) -> Tuple[str, dict]:
...
@tool(response_format="content_and_artifact")
def default_scenarios_wrapped(
    sigma_levels: Sequence[float], mask_levels: Sequence[float]
) -> Tuple[str, dict]:
...
@tool(response_format="content_and_artifact")
def evaluate_robustness_wrapped(
    model_name: str, predict: Callable[[np.ndarray], np.ndarray], X: np.ndarray, y: np.ndarray
) -> Tuple[str, dict]:
...
def make_robustness_agent(
    ...
    def run_react_agent(state: GraphState):
        ...
        messages = [
            (
                "system",
                "You are the F5 agent. Use the available tools to complete the user's request.",
            )
        ] + list(base)
...
class RobustnessTestAgent(BaseAgent):
    ...
    def _make_compiled_graph(self):
        self.response = None
        return make_robustness_agent(**self._params)
