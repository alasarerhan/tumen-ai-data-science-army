...
tool layer (``ai_data_science_team.tools.hpo``) with
LangChain ``@tool`` decorators and exposes the standard
``make_hpo_agent`` factory + ``E2Agent`` OO
...
def suggest_default_search_space_wrapped(engine: str, task_type: str) -> Tuple[str, dict]:
    ...
    try:
        ...
        return f"Tool e2_suggest_default_search_space failed: {exc}", {
            "suggest_default_search_space": kwargs,
            "args": kwargs,
            "result": None,
            "content": f"error: {exc}",
        }
...
class HPOAgent(BaseAgent):
    ...
    def _make_compiled_graph(self):
        self.response = None
        return make_hpo_agent(**self._params)
