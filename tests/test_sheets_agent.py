"""
Tests for ``ai_data_science_team.agents.sheets_agent`` (H5 layer).
...
Phase-6 integration tests covering:
  ...
  3. ``make_sheets_agent`` factory returns a compiled graph with the
     expected node wiring
  4. ``SheetsSourceAgent`` (BaseAgent subclass) constructs, exposes accessors,
...
class _StubModel:
    ...
    ``make_sheets_agent`` calls ``create_agent(model, ...)`` to bind tools;
...
def _agent_with_no_op_react(monkeypatch) -> SheetsSourceAgent:
    ...
    return SheetsSourceAgent(model=_StubModel())
...
class TestModuleSurface:
    ...
    def test_tool_names_match_registry(self):
        ...
        wrapper_names = {t.name for t in SHEETS_SOURCE_TOOLS}
    ...
    def test_all_individual_tools_exported(self):
        ...
        mod = sys.modules["ai_data_science_team.agents.sheets_agent"]
        wrapper_names = ["build_sheets_connector_wrapped"]
...
class TestBuildSheetsConnector:
    def test_direct_call_returns_tuple(self):
        ...
        from ai_data_science_team.agents.sheets_agent import build_sheets_connector_wrapped
        kwargs = {
            "config": None,
        }
        try:
            result = build_sheets_connector_wrapped.invoke(kwargs)
...
class TestMakeSheetsSourceAgent:
    def test_factory_returns_compiled_graph(self):
        app = make_sheets_agent(model=_StubModel())
    ...
    def test_factory_with_checkpointer(self):
        ...
        app = make_sheets_agent(model=_StubModel(), checkpointer=InMemorySaver())
...
class TestSheetsSourceAgent:
    def test_init_compiles_graph(self):
        agent = SheetsSourceAgent(model=_StubModel())
