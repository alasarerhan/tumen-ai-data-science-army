...
Phase-5 agent wrapper for spec H6.  Wraps the deterministic
tool layer (``ai_data_science_team.tools.object_storage``) with
LangChain ``@tool`` decorators and exposes the standard
``make_object_storage_agent`` factory + ``H6Agent`` OO
wrapper, following the same pattern as ABTestingAgent and
PowerAnalysisAgent.

Node type: ``source.object_storage``
"""
...
from ai_data_science_team.tools.object_storage import (  # noqa: E402, F401
...
AGENT_NAME = "object_storage_agent"
...
def build_object_storage_connector_wrapped(config: ConnectorConfig) -> Tuple[str, dict]:
    ...
    """
    logger.info("    * Tool: h6_build_object_storage_connector")
    kwargs = {"config": config}
    try:
        result = build_object_storage_connector(**kwargs)
    except Exception as exc:
        return f"Tool h6_build_object_storage_connector failed: {exc}", {
            "build_object_storage_connector": kwargs,
            "args": kwargs,
            "result": None,
            "content": f"error: {exc}",
        }
    content = "h6_build_object_storage_connector: ok"
    return content, {
        "build_object_storage_connector": kwargs,
        "args": kwargs,
        "result": result,
...
OBJECT_STORAGE_TOOLS = [
    build_object_storage_connector_wrapped,
]
...
def make_object_storage_agent(
    ...
    """Build the LangGraph StateGraph for the H6 agent."""
    ...
    def run_react_agent(state: GraphState):
        logger.info("    * RUN REACT AGENT FOR H6")
        base = state.get("messages") or [("user", state.get("user_instructions"))]
        messages = [
            (
                "system",
                "You are the H6 agent. Use the available tools to complete the user's request.",
            )
        ] + list(base)
    ...
    def post_process(state: GraphState):
        logger.info("    * POST-PROCESSING H6 RESULTS")
...
class ObjectStorageAgent(BaseAgent):
    """OO wrapper for the H6 agent (node type ``source.object_storage``)."""
    ...
    def _make_compiled_graph(self):
        self.response = None
        return make_object_storage_agent(**self._params)
