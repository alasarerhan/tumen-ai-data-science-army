"""
Tests for ``ai_data_science_team.agents.h6_object_storage_agent`` (H6 layer).
...
Phase-6 integration tests covering:
  ...
  3. ``make_h6_object_storage_agent`` factory returns a compiled graph with the
     expected node wiring
  4. ``H6Agent`` (BaseAgent subclass) constructs, exposes accessors,
...
from ai_data_science_team.agents.h6_object_storage_agent import (
    build_object_storage_connector,
    H6Agent,
    make_h6_object_storage_agent,
    AGENT_NAME,
    NODE_TYPE,
    H6_TOOLS,
)
...
class _StubModel:
    ...
    ``make_h6_object_storage_agent`` calls ``create_agent(model, ...)`` to bind tools;
...
def _agent_with_no_op_react(monkeypatch) -> H6Agent:
    ...
    return H6Agent(model=_StubModel())
...
class TestModuleSurface:
    ...
    def test_tool_names_match_registry(self):
        ...
        wrapper_names = {t.name for t in H6_TOOLS}
    ...
    def test_all_individual_tools_exported(self):
        ...
        import sys
        mod = sys.modules["ai_data_science_team.agents.h6_object_storage_agent"]
        wrapper_names = ["h6_build_object_storage_connector_wrapped"]
...
class TestBuildObjectStorageConnector:
    def test_direct_call_returns_tuple(self):
        """Call build_object_storage_connector via its @tool wrapper (LLM-free)."""
        from ai_data_science_team.agents.h6_object_storage_agent import h6_build_object_storage_connector_wrapped
        kwargs = {
            "config": None,
        }
        try:
            result = h6_build_object_storage_connector_wrapped.invoke(kwargs)
...
class TestMakeH6Agent:
    def test_factory_returns_compiled_graph(self):
        from langgraph.graph import StateGraph
        app = make_h6_object_storage_agent(model=_StubModel())
    ...
    def test_factory_with_checkpointer(self):
        ...
        app = make_h6_object_storage_agent(model=_StubModel(), checkpointer=InMemorySaver())
...
class TestH6Agent:
    def test_init_compiles_graph(self):
        agent = H6Agent(model=_StubModel())
