# E4 — NLP / Text Analytics

> **Öncelik:** P1 · **Faz:** 2 · **Kapsam:** metin sınıflandırma + konu modelleme

## 1. Amaç & Kullanıcı Hikâyeleri

Metin sınıflandırma, duygu, konu modelleme (BERTopic), NER, embedding çıkarımı.

**Kabul senaryoları:**

1. Metin sınıflandırma, duygu, konu modelleme, NER, embedding.
2. Konu/duygu dağılım görselleri + örnek metin açıklamaları.
3. HuggingFace transformer pipeline kullanımı.

## 2. Backend Tasarımı

**Agent/Servis:** `ai_data_science_team/agents/e4_agent.py` — `langgraph` react-agent; deterministic çekirdek aynı paketin `tools/` altında.

**Node tipi:**
```json
{
  "type": "model.train.nlp",
  "config": {
...
**Veri modeli:**
- `e4_runs (run_id, status, params_json, result_json, created_at)`
- `e4_artifacts (artifact_ref, run_id, kind, payload)`
...
**Konum:** Designer'da NLP node + sonuç ekranı
...
- İlgili node tipi `model.train.nlp` platform-api-app'te handler'a bağlı.
