"""GERÇEK model-driven object_storage_agent tool doğrulaması (PM kararı: stub test yok).
...
def test_build_object_storage_connector_real(llm_or_skip, llm_model):
    """build_object_storage_connector Pydantic ConnectorConfig alır; in-memory nesne döndürür."""
    tool = build_object_storage_connector_wrapped
    result = _invoke(
        tool, llm_model,
        "build_object_storage_connector tool'unu TEK çağrı ile çağır. "
        "config = {\"kind\": \"object_storage\", \"name\": \"s3_demo\", \"params\": {}}.",
    )
    ...
    assert "ok" in s or "failed" in s or "error" in s, (
        f"build_object_storage_connector beklenen tool execution çıktısı üretmedi: {s[:300]}"
    )
