...
tool layer (``ai_data_science_team.tools.data_diff``) with
LangChain ``@tool`` decorators and exposes the standard
``make_data_diff_agent`` factory + ``J13Agent`` OO
...
@tool(response_format="content_and_artifact")
def numeric_shift_wrapped(left: pd.Series, right: pd.Series) -> Tuple[str, dict]:
...
@tool(response_format="content_and_artifact")
def schema_delta_wrapped(left: pd.DataFrame, right: pd.DataFrame) -> Tuple[str, dict]:
...
@tool(response_format="content_and_artifact")
def key_set_diff_wrapped(left: pd.DataFrame, right: pd.DataFrame, key: str) -> Tuple[str, dict]:
...
@tool(response_format="content_and_artifact")
def diff_summary_wrapped(left: pd.DataFrame, right: pd.DataFrame) -> Tuple[str, dict]:
...
@tool(response_format="content_and_artifact")
def diff_payload_wrapped(left: pd.DataFrame, right: pd.DataFrame) -> Tuple[str, dict]:
...
class DataDiffAgent(BaseAgent):
    ...
    def _make_compiled_graph(self):
        self.response = None
        return make_data_diff_agent(**self._params)
