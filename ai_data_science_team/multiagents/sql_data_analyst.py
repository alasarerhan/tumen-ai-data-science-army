...
from ai_data_science_team.agents import DataVisualizationAgent, SQLDatabaseAgent  # noqa: E402, F401
...
def make_sql_data_analyst(
    ...
    def prepare_messages(state: PrimaryState):
        ...
        if not msgs:
            ...
            msgs = [
                ("system", system_hint),  # type: ignore[list-item]
                ("user", ui),  # type: ignore[list-item]
            ]
        if not ui:
            for msg in reversed(msgs):
                if getattr(msg, "type", None) == "human" or getattr(msg, "role", None) == "user":
                    ui = msg.content  # type: ignore[assignment]
                    break
    ...
    def finalize_output(state: PrimaryState):
        ...
        msgs = state.get("messages", [])
        msgs = list(msgs) + [ai_msg]  # type: ignore[operator]
