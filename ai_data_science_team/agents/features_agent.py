...
tool layer (``ai_data_science_team.tools.features``) with
LangChain ``@tool`` decorators and exposes the standard
``make_features_agent`` factory + ``D2Agent`` OO
...
@tool(response_format="content_and_artifact")
def select_filter_wrapped(df: pd.DataFrame, target: pd.Series) -> Tuple[str, dict]:
...
@tool(response_format="content_and_artifact")
def select_wrapper_wrapped(df: pd.DataFrame, target: pd.Series) -> Tuple[str, dict]:
...
@tool(response_format="content_and_artifact")
def select_embedded_wrapped(df: pd.DataFrame, target: pd.Series) -> Tuple[str, dict]:
...
@tool(response_format="content_and_artifact")
def detect_leakage_wrapped(df: pd.DataFrame, target: pd.Series) -> Tuple[str, dict]:
...
@tool(response_format="content_and_artifact")
def multicollinearity_report_wrapped(df: pd.DataFrame) -> Tuple[str, dict]:
    ...
    try:
        ...
        return f"Tool d2_multicollinearity_report failed: {exc}", {
            "multicollinearity_report": kwargs,
            "args": kwargs,
            "result": None,
            "content": f"error: {exc}",
        }
...
@tool(response_format="content_and_artifact")
def select_feature_wrapped(df: pd.DataFrame, target: pd.Series) -> Tuple[str, dict]:
...
class FeatureSelectionAgent(BaseAgent):
    ...
    def _make_compiled_graph(self):
        self.response = None
        return make_features_agent(**self._params)
