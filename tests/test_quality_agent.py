"""quality_agent modül yüzeyi + fabrika gerçek testleri (LLM-free).

Bu dosya yalnızca modülün export ettiği sabit isimleri (AGENT_NAME, NODE_TYPE,
QUALITY_TOOLS), tool wrapper sınıflarının LangChain ``StructuredTool`` sözleşmesine
uyumluluğunu, ve ``make_quality_agent`` fabrikasının beklenen LangGraph node'larını
ürettiğini doğrular. **LLM çağrısı içermez** — bu yüzden hızlıdır ve LLM yokken
koşabilir.

Tool davranış testleri (3 tool) ``tests/llm/test_quality_agent_tool_calling.py``
...
def test_constants():
    """AGENT_NAME lowercase spec_id + '_agent'; NODE_TYPE 'data.validate'."""
    assert AGENT_NAME == "quality_agent"
...
# ---------------------------------------------------------------------------
# 3. make_quality_agent fabrika wiring (LLM-free, no monkey-patch)
# ---------------------------------------------------------------------------


class _CallableModel:
    """``make_quality_agent`` yalnızca ``model`` argümanının callability'sini
    kontrol eder; bu stub, gerçek bir LLM çağrısı yapmaz. Aşağıdaki
    ``make_quality_agent`` testleri yalnızca graf düğümlerinin varlığını kontrol eder,
...
def test_make_quality_agent_builds_graph():
    """make_quality_agent bir LangGraph StateGraph döndürmeli ve en azından
    reakt-agent + post_process node'larını içermeli."""
    graph = make_quality_agent(model=_CallableModel())
    ...
    assert len(node_ids) >= 1, f"make_quality_agent boş graph döndürdü: {node_ids}"
