...
tool layer (``ai_data_science_team.tools.investigation``) with
LangChain ``@tool`` decorators and exposes the standard
``make_investigation_agent`` factory + ``J1Agent`` OO
...
from ai_data_science_team.tools.investigation import (  # noqa: E402, F401
    detect_change,
    investigate,
    isolate_dimension,
    narrate,
...
def detect_change_wrapped() -> Tuple[str, dict]:
...
@tool(response_format="content_and_artifact")
def isolate_dimension_wrapped() -> Tuple[str, dict]:
...
@tool(response_format="content_and_artifact")
def quantify_contributors_wrapped() -> Tuple[str, dict]:
...
@tool(response_format="content_and_artifact")
def narrate_wrapped() -> Tuple[str, dict]:
...
@tool(response_format="content_and_artifact")
def investigate_wrapped() -> Tuple[str, dict]:
...
def make_investigation_agent(
    ...
    def run_react_agent(state: GraphState):
        ...
        messages = [
            (
                "system",
                "You are the J1 agent. Use the available tools to complete the user's request.",
            )
        ] + list(base)
...
class InvestigationAgent(BaseAgent):
    ...
    def _make_compiled_graph(self):
        self.response = None
        return make_investigation_agent(**self._params)
