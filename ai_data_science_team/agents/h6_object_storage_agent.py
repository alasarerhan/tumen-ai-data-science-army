...
tool layer (``ai_data_science_team.tools.h6_object_storage``) with
LangChain ``@tool`` decorators and exposes the standard
``make_h6_object_storage_agent`` factory + ``H6Agent`` OO
...
from ai_data_science_team.tools.h6_object_storage import (
...
H6_TOOLS = [
    h6_build_object_storage_connector_wrapped,
]
...
def make_h6_object_storage_agent(
    model: Any,
...
class H6Agent(BaseAgent):
    ...
    def _make_compiled_graph(self):
        self.response = None
        return make_h6_object_storage_agent(**self._params)
