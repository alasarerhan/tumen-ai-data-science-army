"""rest_api_agent modül yüzeyi + fabrika gerçek testleri (LLM-free).

Bu dosya yalnızca modülün export ettiği sabit isimleri (AGENT_NAME, NODE_TYPE,
REST_API_TOOLS), tool wrapper sınıflarının LangChain ``StructuredTool`` sözleşmesine
uyumluluğunu, ve ``make_rest_api_agent`` fabrikasının beklenen LangGraph node'larını
ürettiğini doğrular. **LLM çağrısı içermez** — bu yüzden hızlıdır ve LLM yokken
koşabilir.

Tool davranış testleri (1 tool) ``tests/llm/test_rest_api_agent_tool_calling.py``
...
def test_constants():
    """AGENT_NAME lowercase spec_id + '_agent'; NODE_TYPE 'source.rest_api'."""
    assert AGENT_NAME == "rest_api_agent"
    assert NODE_TYPE == "source.rest_api"


def test_tool_registry_non_empty():
    assert len(REST_API_TOOLS) >= 1
    wrapper_names = {t.name for t in REST_API_TOOLS}
...
def test_all_individual_tools_exported():
    ...
    mod = sys.modules["ai_data_science_team.agents.rest_api_agent"]
    wrapper_names = ["build_rest_connector_wrapped"]
...
# ---------------------------------------------------------------------------
# 3. make_rest_api_agent fabrika wiring (LLM-free, no monkey-patch)
# ---------------------------------------------------------------------------


class _CallableModel:
    """``make_rest_api_agent`` yalnızca ``model`` argümanının callability'sini
    kontrol eder; bu stub, gerçek bir LLM çağrısı yapmaz. Aşağıdaki
    ``make_rest_api_agent`` testleri yalnızca graf düğümlerinin varlığını kontrol eder,
...
def test_make_rest_api_agent_builds_graph():
    """make_rest_api_agent bir LangGraph StateGraph döndürmeli ve en azından
    reakt-agent + post_process node'larını içermeli."""
    graph = make_rest_api_agent(model=_CallableModel())
    ...
    assert len(node_ids) >= 1, f"make_rest_api_agent boş graph döndürdü: {node_ids}"
