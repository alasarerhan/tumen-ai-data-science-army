...
tool layer (``ai_data_science_team.tools.evaluation_ext``) with
LangChain ``@tool`` decorators and exposes the standard
``make_evaluation_ext_agent`` factory + ``F1Agent`` OO
...
from ai_data_science_team.tools.evaluation_ext import (  # noqa: E402, F401
...
AGENT_NAME = "evaluation_ext_agent"
...
def evaluate_calibration_wrapped(
    y_true: Sequence[int], y_prob: Sequence[float], n_bins: int
) -> Tuple[str, dict]:
...
@tool(response_format="content_and_artifact")
def optimize_threshold_wrapped(y_true: Sequence[int], y_prob: Sequence[float]) -> Tuple[str, dict]:
...
@tool(response_format="content_and_artifact")
def evaluate_segments_wrapped(
    df: pd.DataFrame, y_true: Sequence[int], y_pred: Sequence[int], segment_columns: Sequence[str]
) -> Tuple[str, dict]:
...
def make_evaluation_ext_agent(
    ...
    def run_react_agent(state: GraphState):
        ...
        messages = [
            (
                "system",
                "You are the F1 agent. Use the available tools to complete the user's request.",
            )
        ] + list(base)
...
class EvaluationExtAgent(BaseAgent):
    ...
    def _make_compiled_graph(self):
        self.response = None
        return make_evaluation_ext_agent(**self._params)
