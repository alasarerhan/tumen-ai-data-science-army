...
def relocate_imports_inside_functions(code_text):
    """
    Relocates all import statements in a given Python function and moves them inside the funciton definition.
    ...
    if match:
        ...
        modified_code = (
            code_without_imports[:function_start] + imports_code + code_without_imports[function_start:]
...
def add_comments_to_top(code_text, agent_name="data_wrangler"):
    ...
    ----------
    code_text: str
        The python code to be committed.
    ...
    #! Join the header with new lines, then prepend to the existing code_text
...
def format_recommended_steps(raw_text:str, heading:str = "# Recommended Steps:") -> str:
    ...
    for line in lines:
        #! If this line *is exactly* the heding, check if we've seen it already
...
def get_generic_summary(report_dict: dict, code_lang = "python") -> str:
    ...
    #! 1) Grab rhe report title (or default)
    ...
    for key, value in report_dict.items():
        ...
        #! 3) Check if it is code or function
        key_lower = key.lower()
...
def remove_consecutive_duplicates(messages):
    ...
    for msg in messages:
        ...
        prev_message = msg.content #! Update previous message tp current
