...
tool layer (``ai_data_science_team.tools.a3_bayesian``) with
LangChain ``@tool`` decorators and exposes the standard
``make_a3_bayesian_agent`` factory + ``A3Agent`` OO
...
A3_TOOLS = [
    a3_beta_posterior_wrapped,
    a3_bayes_decision_wrapped,
    a3_normal_means_posterior_wrapped,
]
...
class A3Agent(BaseAgent):
    ...
    def _make_compiled_graph(self):
        self.response = None
        return make_a3_bayesian_agent(**self._params)
