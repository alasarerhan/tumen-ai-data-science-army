"""
Tests for ``ai_data_science_team.agents.h2_bigquery_agent`` (H2 layer).
...
Phase-6 integration tests covering:
  ...
  3. ``make_h2_bigquery_agent`` factory returns a compiled graph with the
     expected node wiring
  4. ``H2Agent`` (BaseAgent subclass) constructs, exposes accessors,
...
from ai_data_science_team.agents.h2_bigquery_agent import (
    build_bigquery_connector,
    H2Agent,
    make_h2_bigquery_agent,
    AGENT_NAME,
    NODE_TYPE,
    H2_TOOLS,
)
...
class _StubModel:
    ...
    ``make_h2_bigquery_agent`` calls ``create_agent(model, ...)`` to bind tools;
...
def _agent_with_no_op_react(monkeypatch) -> H2Agent:
    ...
    return H2Agent(model=_StubModel())
...
class TestModuleSurface:
    ...
    def test_tool_names_match_registry(self):
        ...
        wrapper_names = {t.name for t in H2_TOOLS}
    ...
    def test_all_individual_tools_exported(self):
        ...
        import sys
        mod = sys.modules["ai_data_science_team.agents.h2_bigquery_agent"]
        wrapper_names = ["h2_build_bigquery_connector_wrapped"]
...
class TestBuildBigqueryConnector:
    def test_direct_call_returns_tuple(self):
        ...
        from ai_data_science_team.agents.h2_bigquery_agent import h2_build_bigquery_connector_wrapped
        kwargs = {
            "config": None,
        }
        try:
            result = h2_build_bigquery_connector_wrapped.invoke(kwargs)
...
class TestMakeH2Agent:
    def test_factory_returns_compiled_graph(self):
        from langgraph.graph import StateGraph
        app = make_h2_bigquery_agent(model=_StubModel())
    ...
    def test_factory_with_checkpointer(self):
        ...
        app = make_h2_bigquery_agent(model=_StubModel(), checkpointer=InMemorySaver())
...
class TestH2Agent:
    def test_init_compiles_graph(self):
        agent = H2Agent(model=_StubModel())
