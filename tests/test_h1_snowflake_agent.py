"""
Tests for ``ai_data_science_team.agents.h1_snowflake_agent`` (H1 layer).
...
Phase-6 integration tests covering:
  ...
  3. ``make_h1_snowflake_agent`` factory returns a compiled graph with the
     expected node wiring
  4. ``H1Agent`` (BaseAgent subclass) constructs, exposes accessors,
...
from ai_data_science_team.agents.h1_snowflake_agent import (
    build_snowflake_connector,
    H1Agent,
    make_h1_snowflake_agent,
    AGENT_NAME,
    NODE_TYPE,
    H1_TOOLS,
)
...
class _StubModel:
    ...
    ``make_h1_snowflake_agent`` calls ``create_agent(model, ...)`` to bind tools;
...
def _agent_with_no_op_react(monkeypatch) -> H1Agent:
    ...
    return H1Agent(model=_StubModel())
...
class TestModuleSurface:
    ...
    def test_tool_names_match_registry(self):
        ...
        wrapper_names = {t.name for t in H1_TOOLS}
    ...
    def test_all_individual_tools_exported(self):
        ...
        import sys
        mod = sys.modules["ai_data_science_team.agents.h1_snowflake_agent"]
        wrapper_names = ["h1_build_snowflake_connector_wrapped"]
...
class TestBuildSnowflakeConnector:
    def test_direct_call_returns_tuple(self):
        ...
        from ai_data_science_team.agents.h1_snowflake_agent import h1_build_snowflake_connector_wrapped
        kwargs = {
            "config": None,
        }
        try:
            result = h1_build_snowflake_connector_wrapped.invoke(kwargs)
...
class TestMakeH1Agent:
    def test_factory_returns_compiled_graph(self):
        from langgraph.graph import StateGraph
        app = make_h1_snowflake_agent(model=_StubModel())
    ...
    def test_factory_with_checkpointer(self):
        ...
        app = make_h1_snowflake_agent(model=_StubModel(), checkpointer=InMemorySaver())
...
class TestH1Agent:
    def test_init_compiles_graph(self):
        agent = H1Agent(model=_StubModel())
