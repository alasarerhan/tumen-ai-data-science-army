"""GERÇEK model-driven sheets_agent tool doğrulaması (PM kararı: stub test yok).
...
def test_build_sheets_connector_real(llm_or_skip, llm_model):
    """build_sheets_connector Pydantic ConnectorConfig alır; in-memory nesne döndürür."""
    tool = build_sheets_connector_wrapped
    result = _invoke(
        tool,
        llm_model,
        "build_sheets_connector tool'unu TEK çağrı ile çağır. "
        'config = {"kind": "sheets", "name": "sheet_demo", "params": {}}.',
    )
    ...
    assert "ok" in s or "failed" in s or "error" in s, (
        f"build_sheets_connector beklenen tool execution çıktısı üretmedi: {s[:300]}"
    )
