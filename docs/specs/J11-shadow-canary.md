# J11 — Shadow/Canary Deployment

> **Öncelik:** P2 · **Faz:** 3 · **Kapsam:** kısmi trafik canary

## 1. Amaç & Kullanıcı Hikâyeleri

Yeni modeli kısmi/gölge trafikte canlı karşılaştırma + otomatik rollback eşiği.

**Kabul senaryoları:**

1. Yeni modeli kısmi/gölge trafikte canlı karşılaştırma.
2. Otomatik rollback eşiği.
3. Canary yüzde slider'ı, canlı trafik karşılaştırma grafiği.

## 2. Backend Tasarımı

**Agent/Servis:** `ai_data_science_team/agents/j11_agent.py` — `langgraph` react-agent; deterministic çekirdek aynı paketin `tools/` altında.
...
**API endpoint'leri:**
- `POST /deploy/shadow`
- `GET /deploy/shadow/{id}`
- `GET /deploy/shadow`

**Veri modeli:**
- `j11_runs (run_id, status, params_json, result_json, created_at)`
- `j11_artifacts (artifact_ref, run_id, kind, payload)`
...
**Konum:** K2 deploy panelinde canary slider'ı
...
- İlgili node tipi `deploy.shadow` platform-api-app'te handler'a bağlı.
