"""features_agent modül yüzeyi + fabrika gerçek testleri (LLM-free).

Bu dosya yalnızca modülün export ettiği sabit isimleri (AGENT_NAME, NODE_TYPE,
FEATURE_SELECTION_TOOLS), tool wrapper sınıflarının LangChain ``StructuredTool`` sözleşmesine
uyumluluğunu, ve ``make_features_agent`` fabrikasının beklenen LangGraph node'larını
...
def test_constants():
    """AGENT_NAME lowercase spec_id + '_agent'; NODE_TYPE 'feature.select'."""
...
EXPECTED_WRAPPERS = [
    "filter_scores_wrapped",
    "select_filter_wrapped",
    "select_wrapper_wrapped",
    "select_embedded_wrapped",
    "detect_leakage_wrapped",
    "multicollinearity_report_wrapped",
    "select_feature_wrapped",
]
...
# ---------------------------------------------------------------------------
# 3. make_features_agent fabrika wiring (LLM-free, no monkey-patch)
# ---------------------------------------------------------------------------


class _CallableModel:
    """``make_features_agent`` yalnızca ``model`` argümanının callability'sini
    kontrol eder; bu stub, gerçek bir LLM çağrısı yapmaz. Aşağıdaki
    ``make_features_agent`` testleri yalnızca graf düğümlerinin varlığını kontrol eder,
...
def test_make_features_agent_builds_graph():
    """make_features_agent bir LangGraph StateGraph döndürmeli ve en azından
    reakt-agent + post_process node'larını içermeli."""
    graph = make_features_agent(model=_CallableModel())
    ...
    assert len(node_ids) >= 1, f"make_features_agent boş graph döndürdü: {node_ids}"
