...
class TestExpectationSuiteFromTemplate:
    def test_direct_call_returns_tuple(self):
        ...
        try:
            result = b2_expectation_suite_from_template_wrapped.invoke(kwargs)
...
class TestValidateAgainstSuite:
    def test_direct_call_returns_tuple(self):
        ...
        try:
            result = b2_validate_against_suite_wrapped.invoke(kwargs)
        except Exception as exc:
            # If the placeholder kwargs are insufficient, the tool may raise.
            # We still accept the call attempt as evidence the wrapper is wired up.
            assert isinstance(exc, Exception)
...
class TestSummariseSuiteRun:
    def test_direct_call_returns_tuple(self):
        ...
        try:
            result = b2_summarise_suite_run_wrapped.invoke(kwargs)
        except Exception as exc:
            # If the placeholder kwargs are insufficient, the tool may raise.
            # We still accept the call attempt as evidence the wrapper is wired up.
            assert isinstance(exc, Exception)
