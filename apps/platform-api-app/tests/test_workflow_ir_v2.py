...
def _valid_ir_v2_spec() -> dict:
    return {
        ...
        "inputs": [
            {"name": "dataset", "artifact_type": "dataset", "source": "run.input_artifact_ids"}
        ],
        "nodes": [
            {
                "id": "profile",
...
def test_run_with_workflow_spec_creates_node_execution_state(admin_client):
    ...
    with patch(
        ...
        new=AsyncMock(return_value="prefect-ir-v2-run"),
    ):
    ...
    assert [item["node_id"] for item in nodes.json()["items"]] == [
        "profile",
        "clean",
        "features",
        "train",
...
def test_retry_failed_node_and_resume_run(admin_client):
    ...
    with patch(
        "platform_api.routes.runs.create_orchestration_run_id",
        new=AsyncMock(return_value="prefect-retry-run"),
    ):
        run = client.post(
            f"/v1/runs?workspace_id={workspace.id}",
            json={
                "workspace_id": str(workspace.id),
                "flow_key": "retry-workflow",
                "workflow_spec_id": str(workflow.id),
            },
        ).json()
    ...
    node = (
        ...
        .filter_by(workflow_run_id=uuid.UUID(run["id"]), node_id="profile")
        .one()
    )
    node.status = "failed"
    ...
    db.commit()
    resumed = client.post(
        f"/v1/runs/{run['id']}/resume?workspace_id={workspace.id}",
        json={"workspace_id": str(workspace.id)},
    )
...
def test_dataset_profile_executor_reads_dataset_and_writes_profile(
    ...
    result = get_default_node_executors()["dataset.profile"](
        NodeExecutionContext(db=db, run=run, node=node)
    )
    ...
    assert (
        tmp_path / "artifacts" / "workflow-runs" / str(run.id) / "profile" / "profile.json"
    ).exists()
...
def test_worker_records_safe_agent_execution_trace(admin_client):
    ...
    node = WorkflowNodeExecution(
        ...
        inputs_json=json.dumps(
            {
                "config": {"instruction": "summarize", "password": "do-not-store"},
                "api_token": "secret",
            }
        ),
        logs_json="[]",
    ...
    def _executor(ctx: NodeExecutionContext) -> dict:
        return {
            "outputs": {
                ...
                "tool_calls": [
                    {"name": "sql.query", "args": {"query": "select 1", "password": "do-not-store"}}
                ],
                "token_usage": {
                    "prompt_tokens": 120,
    ...
    assert {
        ...
        "evaluation_summary",
        "version_metadata",
    }.issubset(set(body["output_summary"]["output_keys"]))
...
def test_worker_records_failed_agent_trace_without_secret_leak(seeded_db):
    ...
    status = WorkflowWorker(node_executors={"failing.agent": _executor})._execute_node(
        db, run, node
    )
    db.commit()
