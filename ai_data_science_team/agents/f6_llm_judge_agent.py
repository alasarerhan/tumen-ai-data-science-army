...
tool layer (``ai_data_science_team.tools.f6_llm_judge``) with
LangChain ``@tool`` decorators and exposes the standard
``make_f6_llm_judge_agent`` factory + ``F6Agent`` OO
...
from ai_data_science_team.tools.f6_llm_judge import (
    JudgeScore,
    judge_batch,
    judge_output,
)
...
F6_TOOLS = [
    f6_judge_output_wrapped,
    f6_judge_batch_wrapped,
]
...
class F6Agent(BaseAgent):
    ...
    def _make_compiled_graph(self):
        self.response = None
        return make_f6_llm_judge_agent(**self._params)
