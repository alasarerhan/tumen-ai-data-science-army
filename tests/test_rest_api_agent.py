"""
Tests for ``ai_data_science_team.agents.rest_api_agent`` (H7 layer).
...
Phase-6 integration tests covering:
  ...
  3. ``make_rest_api_agent`` factory returns a compiled graph with the
     expected node wiring
  4. ``RESTAPISourceAgent`` (BaseAgent subclass) constructs, exposes accessors,
...
class _StubModel:
    ...
    ``make_rest_api_agent`` calls ``create_agent(model, ...)`` to bind tools;
...
def _agent_with_no_op_react(monkeypatch) -> RESTAPISourceAgent:
    ...
    return RESTAPISourceAgent(model=_StubModel())
...
class TestBuildRestConnector:
    def test_direct_call_returns_tuple(self):
        ...
        from ai_data_science_team.agents.rest_api_agent import build_rest_connector_wrapped
        kwargs = {
            "config": None,
        }
        try:
            result = build_rest_connector_wrapped.invoke(kwargs)
...
class TestMakeRESTAPISourceAgent:
    def test_factory_returns_compiled_graph(self):
        app = make_rest_api_agent(model=_StubModel())
    ...
    def test_factory_with_checkpointer(self):
        ...
        app = make_rest_api_agent(model=_StubModel(), checkpointer=InMemorySaver())
...
class TestRESTAPISourceAgent:
    def test_init_compiles_graph(self):
        agent = RESTAPISourceAgent(model=_StubModel())
