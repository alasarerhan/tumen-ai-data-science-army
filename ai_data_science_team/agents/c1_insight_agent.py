...
tool layer (``ai_data_science_team.tools.c1_insight``) with
LangChain ``@tool`` decorators and exposes the standard
``make_c1_insight_agent`` factory + ``C1Agent`` OO
...
C1_TOOLS = [
    c1_find_anomalies_wrapped,
    c1_find_strong_correlations_wrapped,
    c1_find_skewness_wrapped,
    c1_find_missing_patterns_wrapped,
    c1_find_class_imbalance_wrapped,
    c1_find_constants_and_outliers_wrapped,
    c1_mine_insights_wrapped,
]
...
class C1Agent(BaseAgent):
    ...
    def _make_compiled_graph(self):
        self.response = None
        return make_c1_insight_agent(**self._params)
