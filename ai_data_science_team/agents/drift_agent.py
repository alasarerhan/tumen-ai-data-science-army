...
tool layer (``ai_data_science_team.tools.drift``) with
LangChain ``@tool`` decorators and exposes the standard
``make_drift_agent`` factory + ``G1Agent`` OO
...
@tool(response_format="content_and_artifact")
def performance_drift_wrapped(baseline_metric: float, current_metric: float) -> Tuple[str, dict]:
...
class DriftDetectionAgent(BaseAgent):
    ...
    def _make_compiled_graph(self):
        self.response = None
        return make_drift_agent(**self._params)
