...
tool layer (``ai_data_science_team.tools.j12_lineage``) with
LangChain ``@tool`` decorators and exposes the standard
``make_j12_lineage_agent`` factory + ``J12Agent`` OO
...
from ai_data_science_team.tools.j12_lineage import (
    LineageEdge,
    LineageGraph,
    LineageNode,
    add_edge,
...
J12_TOOLS = [
    j12_add_node_wrapped,
    j12_add_edge_wrapped,
    j12_ancestors_wrapped,
    j12_descendants_wrapped,
    j12_render_graph_wrapped,
    j12_node_summary_wrapped,
]
...
class J12Agent(BaseAgent):
    ...
    def _make_compiled_graph(self):
        self.response = None
        return make_j12_lineage_agent(**self._params)
