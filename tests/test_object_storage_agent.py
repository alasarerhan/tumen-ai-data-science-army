"""
Tests for ``ai_data_science_team.agents.object_storage_agent`` (H6 layer).
...
Phase-6 integration tests covering:
  ...
  3. ``make_object_storage_agent`` factory returns a compiled graph with the
     expected node wiring
  4. ``ObjectStorageAgent`` (BaseAgent subclass) constructs, exposes accessors,
...
class _StubModel:
    ...
    ``make_object_storage_agent`` calls ``create_agent(model, ...)`` to bind tools;
...
def _agent_with_no_op_react(monkeypatch) -> ObjectStorageAgent:
    ...
    return ObjectStorageAgent(model=_StubModel())
...
class TestBuildObjectStorageConnector:
    def test_direct_call_returns_tuple(self):
        ...
        from ai_data_science_team.agents.object_storage_agent import build_object_storage_connector_wrapped
        kwargs = {
            "config": None,
        }
        try:
            result = build_object_storage_connector_wrapped.invoke(kwargs)
...
class TestMakeObjectStorageAgent:
    def test_factory_returns_compiled_graph(self):
        app = make_object_storage_agent(model=_StubModel())
    ...
    def test_factory_with_checkpointer(self):
        ...
        app = make_object_storage_agent(model=_StubModel(), checkpointer=InMemorySaver())
...
class TestObjectStorageAgent:
    def test_init_compiles_graph(self):
        agent = ObjectStorageAgent(model=_StubModel())
