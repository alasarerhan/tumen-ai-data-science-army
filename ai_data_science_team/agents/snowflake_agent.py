...
Phase-5 agent wrapper for spec H1.  Wraps the deterministic
tool layer (``ai_data_science_team.tools.snowflake``) with
LangChain ``@tool`` decorators and exposes the standard
``make_snowflake_agent`` factory + ``H1Agent`` OO
wrapper, following the same pattern as ABTestingAgent and
PowerAnalysisAgent.

Node type: ``source.snowflake``
"""
...
def build_snowflake_connector_wrapped(config: ConnectorConfig) -> Tuple[str, dict]:
    ...
    """
    logger.info("    * Tool: h1_build_snowflake_connector")
    kwargs = {"config": config}
    try:
        result = build_snowflake_connector(**kwargs)
    except Exception as exc:
        return f"Tool h1_build_snowflake_connector failed: {exc}", {
            "build_snowflake_connector": kwargs,
            "args": kwargs,
            "result": None,
            "content": f"error: {exc}",
        }
    content = "h1_build_snowflake_connector: ok"
    return content, {
        "build_snowflake_connector": kwargs,
        "args": kwargs,
        "result": result,
...
SNOWFLAKE_SOURCE_TOOLS = [
    build_snowflake_connector_wrapped,
]
...
def make_snowflake_agent(
    ...
    """Build the LangGraph StateGraph for the H1 agent."""
    ...
    def run_react_agent(state: GraphState):
        logger.info("    * RUN REACT AGENT FOR H1")
        base = state.get("messages") or [("user", state.get("user_instructions"))]
        messages = [
            (
                "system",
                "You are the H1 agent. Use the available tools to complete the user's request.",
            )
        ] + list(base)
    ...
    def post_process(state: GraphState):
        logger.info("    * POST-PROCESSING H1 RESULTS")
...
class SnowflakeSourceAgent(BaseAgent):
    """OO wrapper for the H1 agent (node type ``source.snowflake``)."""
    ...
    def _make_compiled_graph(self):
        self.response = None
        return make_snowflake_agent(**self._params)
