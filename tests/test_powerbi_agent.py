"""
Tests for ``ai_data_science_team.agents.powerbi_agent`` (H4 layer).
...
Phase-6 integration tests covering:
  ...
  3. ``make_powerbi_agent`` factory returns a compiled graph with the
     expected node wiring
  4. ``PowerBISourceAgent`` (BaseAgent subclass) constructs, exposes accessors,
...
class _StubModel:
    ...
    ``make_powerbi_agent`` calls ``create_agent(model, ...)`` to bind tools;
...
def _agent_with_no_op_react(monkeypatch) -> PowerBISourceAgent:
    ...
    return PowerBISourceAgent(model=_StubModel())
...
class TestBuildPowerbiConnector:
    def test_direct_call_returns_tuple(self):
        ...
        from ai_data_science_team.agents.powerbi_agent import build_powerbi_connector_wrapped
        kwargs = {
            "config": None,
        }
        try:
            result = build_powerbi_connector_wrapped.invoke(kwargs)
...
class TestMakePowerBISourceAgent:
    def test_factory_returns_compiled_graph(self):
        app = make_powerbi_agent(model=_StubModel())
    ...
    def test_factory_with_checkpointer(self):
        ...
        app = make_powerbi_agent(model=_StubModel(), checkpointer=InMemorySaver())
...
class TestPowerBISourceAgent:
    def test_init_compiles_graph(self):
        agent = PowerBISourceAgent(model=_StubModel())
