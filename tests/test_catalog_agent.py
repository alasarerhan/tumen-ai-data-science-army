"""catalog_agent modül yüzeyi + fabrika gerçek testleri (LLM-free).

Bu dosya yalnızca modülün export ettiği sabit isimleri (AGENT_NAME, NODE_TYPE,
DATA_CATALOG_TOOLS), tool wrapper sınıflarının LangChain ``StructuredTool`` sözleşmesine
uyumluluğunu, ve ``make_catalog_agent`` fabrikasının beklenen LangGraph node'larını
ürettiğini doğrular. **LLM çağrısı içermez** — bu yüzden hızlıdır ve LLM yokken
koşabilir.

Tool davranış testleri (12 tool) ``tests/llm/test_catalog_agent_tool_calling.py``
...
def test_constants():
    """AGENT_NAME lowercase spec_id + '_agent'; NODE_TYPE 'catalog.search'."""
...
def test_all_individual_tools_exported():
    """12 wrapper'ın hepsi modülden erişilebilir ve StructuredTool sözleşmesine sahip."""
...
# ---------------------------------------------------------------------------
# 3. make_catalog_agent fabrika wiring (LLM-free, no monkey-patch)
# ---------------------------------------------------------------------------


class _CallableModel:
    """``make_catalog_agent`` yalnızca ``model`` argümanının callability'sini
    kontrol eder; bu stub, gerçek bir LLM çağrısı yapmaz. Aşağıdaki
    ``make_catalog_agent`` testleri yalnızca graf düğümlerinin varlığını kontrol eder,
...
def test_make_catalog_agent_builds_graph():
    """make_catalog_agent bir LangGraph StateGraph döndürmeli ve en azından
    reakt-agent + post_process node'larını içermeli."""
    graph = make_catalog_agent(model=_CallableModel())
    ...
    assert len(node_ids) >= 1, f"make_catalog_agent boş graph döndürdü: {node_ids}"
