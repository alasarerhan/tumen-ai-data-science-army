...
tool layer (``ai_data_science_team.tools.c5_reports``) with
LangChain ``@tool`` decorators and exposes the standard
``make_c5_reports_agent`` factory + ``C5Agent`` OO
...
C5_TOOLS = [
    c5_get_template_wrapped,
    c5_build_report_wrapped,
    c5_compute_schedule_wrapped,
    c5_render_markdown_wrapped,
]
...
class C5Agent(BaseAgent):
    ...
    def _make_compiled_graph(self):
        self.response = None
        return make_c5_reports_agent(**self._params)
