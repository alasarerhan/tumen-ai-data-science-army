"""
Tests for ``ai_data_science_team.agents.llm_judge_agent`` (F6 layer).
...
Phase-6 integration tests covering:
  ...
  3. ``make_llm_judge_agent`` factory returns a compiled graph with the
     expected node wiring
  4. ``LLMJudgeAgent`` (BaseAgent subclass) constructs, exposes accessors,
...
class _StubModel:
    ...
    ``make_llm_judge_agent`` calls ``create_agent(model, ...)`` to bind tools;
...
def _agent_with_no_op_react(monkeypatch) -> LLMJudgeAgent:
    ...
    return LLMJudgeAgent(model=_StubModel())
...
class TestJudgeOutput:
    def test_direct_call_returns_tuple(self):
        ...
        from ai_data_science_team.agents.llm_judge_agent import judge_output_wrapped
        kwargs = {
            "text": "sample",
        }
        try:
            result = judge_output_wrapped.invoke(kwargs)
...
class TestJudgeBatch:
    def test_direct_call_returns_tuple(self):
        ...
        from ai_data_science_team.agents.llm_judge_agent import judge_batch_wrapped
        kwargs = {
            "items": [],
        }
        try:
            result = judge_batch_wrapped.invoke(kwargs)
        except Exception as exc:
            # If the placeholder kwargs are insufficient, the tool may raise.
            # We still accept the call attempt as evidence the wrapper is wired up.
            assert isinstance(exc, Exception)
...
class TestMakeLLMJudgeAgent:
    def test_factory_returns_compiled_graph(self):
        app = make_llm_judge_agent(model=_StubModel())
    ...
    def test_factory_with_checkpointer(self):
        ...
        app = make_llm_judge_agent(model=_StubModel(), checkpointer=InMemorySaver())
...
class TestLLMJudgeAgent:
    def test_init_compiles_graph(self):
        agent = LLMJudgeAgent(model=_StubModel())
