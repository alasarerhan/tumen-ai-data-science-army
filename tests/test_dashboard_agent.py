"""dashboard_agent modül yüzeyi + fabrika gerçek testleri (LLM-free).

Bu dosya yalnızca modülün export ettiği sabit isimleri (AGENT_NAME, NODE_TYPE,
DASHBOARD_COMPOSER_TOOLS), tool wrapper sınıflarının LangChain ``StructuredTool`` sözleşmesine
uyumluluğunu, ve ``make_dashboard_agent`` fabrikasının beklenen LangGraph node'larını
ürettiğini doğrular. **LLM çağrısı içermez** — bu yüzden hızlıdır ve LLM yokken
koşabilir.

Tool davranış testleri: 4 tool ``Dashboard`` (Pydantic, runtime object) arg alır
→ API test Faz C'de kapsanmalıdır. 1 PURE tool (``make_dashboard``) için
model-driven test ``tests/llm/test_dashboard_agent_tool_calling.py`` altındadır.
PM kararı: mock/stub yok.
"""
...
def test_constants():
    """AGENT_NAME lowercase spec_id + '_agent'; NODE_TYPE 'dashboard.compose'."""
...
# ---------------------------------------------------------------------------
# 3. make_dashboard_agent fabrika wiring (LLM-free, no monkey-patch)
# ---------------------------------------------------------------------------


class _CallableModel:
    """``make_dashboard_agent`` yalnızca ``model`` argümanının callability'sini
    kontrol eder; bu stub, gerçek bir LLM çağrısı yapmaz. Aşağıdaki
    ``make_dashboard_agent`` testleri yalnızca graf düğümlerinin varlığını kontrol eder,
...
def test_make_dashboard_agent_builds_graph():
    """make_dashboard_agent bir LangGraph StateGraph döndürmeli ve en azından
    reakt-agent + post_process node'larını içermeli."""
    graph = make_dashboard_agent(model=_CallableModel())
    ...
    assert len(node_ids) >= 1, f"make_dashboard_agent boş graph döndürdü: {node_ids}"
