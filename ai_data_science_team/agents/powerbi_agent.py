...
Phase-5 agent wrapper for spec H4.  Wraps the deterministic
tool layer (``ai_data_science_team.tools.powerbi``) with
LangChain ``@tool`` decorators and exposes the standard
``make_powerbi_agent`` factory + ``H4Agent`` OO
wrapper, following the same pattern as ABTestingAgent and
PowerAnalysisAgent.

Node type: ``source.powerbi``
"""
...
from ai_data_science_team.tools.powerbi import (  # noqa: E402, F401
...
def build_powerbi_connector_wrapped(config: ConnectorConfig) -> Tuple[str, dict]:
    """Tool wrapper for ``build_powerbi_connector``.

    See underlying tool module.

    Returns a (content, artifact) tuple per the react-agent contract.
    """
    logger.info("    * Tool: h4_build_powerbi_connector")
    kwargs = {"config": config}
    try:
        result = build_powerbi_connector(**kwargs)
    except Exception as exc:
        return f"Tool h4_build_powerbi_connector failed: {exc}", {
            "build_powerbi_connector": kwargs,
            "args": kwargs,
            "result": None,
            "content": f"error: {exc}",
        }
...
POWERBI_SOURCE_TOOLS = [
    build_powerbi_connector_wrapped,
]
...
def make_powerbi_agent(
    ...
    """Build the LangGraph StateGraph for the H4 agent."""
    ...
    def run_react_agent(state: GraphState):
        ...
        messages = [
            (
                "system",
                "You are the H4 agent. Use the available tools to complete the user's request.",
            )
        ] + list(base)
...
class PowerBISourceAgent(BaseAgent):
    """OO wrapper for the H4 agent (node type ``source.powerbi``)."""
    ...
    def _make_compiled_graph(self):
        self.response = None
        return make_powerbi_agent(**self._params)
