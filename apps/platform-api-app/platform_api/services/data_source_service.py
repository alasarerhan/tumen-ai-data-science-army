...
from platform_api.services.secret_store_service import (
    get_data_source_secret,
    put_data_source_secret,
)
...
def _normalize_data_source_not_found_error(exc: Exception) -> Exception:
    if (
        ...
        and getattr(exc, "detail", None) == "DataSource not found"
    ):
...
def _prepare_connection(
    kind: str, connection_uri: str | None, metadata: dict | None
...
def _connection_uri_for_test(db: Session, ds: DataSource) -> str:
    ...
    if metadata.get("trust_server_certificate") is not None:
        query_parts.append(
            f"TrustServerCertificate={'yes' if metadata.get('trust_server_certificate') else 'no'}"
        )
...
def _test_sqlite_connection(parsed) -> dict:
    ...
    if raw_path in {"", "/:memory:"}:
        ...
        conn.close()
        return {
            "message": "SQLite in-memory database is reachable.",
    ...
    try:
        ...
        conn.close()
    return {
        "message": "SQLite database connection succeeded.",
...
def _test_duckdb_connection(parsed) -> dict:
    ...
    conn = duckdb.connect(
        database=str(db_path) if raw_path else ":memory:", read_only=bool(raw_path)
    )
    try:
        ...
        conn.close()
    return {
        "message": "DuckDB connection succeeded.",
        "details": {"database": str(db_path) if raw_path else ":memory:"},
    }
