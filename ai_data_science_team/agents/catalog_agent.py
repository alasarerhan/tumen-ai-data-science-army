...
tool layer (``ai_data_science_team.tools.catalog``) with
LangChain ``@tool`` decorators and exposes the standard
``make_catalog_agent`` factory + ``I2Agent`` OO
...
@tool(response_format="content_and_artifact")
def catalog_tree_wrapped(catalog: Catalog) -> Tuple[str, dict]:
...
@tool(response_format="content_and_artifact")
def add_term_wrapped(catalog: Catalog, term: str) -> Tuple[str, dict]:
...
@tool(response_format="content_and_artifact")
def bind_term_column_wrapped(catalog: Catalog, term: str) -> Tuple[str, dict]:
...
@tool(response_format="content_and_artifact")
def search_wrapped(catalog: Catalog, query: str) -> Tuple[str, dict]:
...
@tool(response_format="content_and_artifact")
def resolve_data_wrapped(catalog: Catalog, term: str) -> Tuple[str, dict]:
...
@tool(response_format="content_and_artifact")
def record_lineage_wrapped(catalog: Catalog) -> Tuple[str, dict]:
...
@tool(response_format="content_and_artifact")
def make_catalog_wrapped() -> Tuple[str, dict]:
...
class DataCatalogAgent(BaseAgent):
    ...
    def _make_compiled_graph(self):
        self.response = None
        return make_catalog_agent(**self._params)
