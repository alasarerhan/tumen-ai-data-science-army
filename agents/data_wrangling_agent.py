...
def make_data_wrangling_agent(
    ...
    def recommend_wrangling_steps(state: GraphState):
        ...
        return {
            "recommended_steps": format_recommended_steps(
                recommended_steps.content.strip(), heading="# Recommended Data Wrangling Steps:"
            ),
            "all_datasets_summary": all_datasets_summary_str,
        }
    ...
    def create_data_wrangler_code(state: GraphState):
        ...
        response = relocate_imports_inside_functions(response)
    ...
    if not bypass_explain_code:
        ...
        def human_review(
            state: GraphState,
        ) -> Command[Literal["recommend_wrangling_steps", "explain_data_wrangler_code"]]:
            return node_func_human_review(
                state=state,
