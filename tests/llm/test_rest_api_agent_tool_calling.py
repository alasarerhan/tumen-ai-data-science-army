"""GERÇEK model-driven rest_api_agent tool doğrulaması (PM kararı: stub test yok).
...
def test_build_rest_connector_real(llm_or_skip, llm_model):
    """build_rest_connector Pydantic ConnectorConfig alır; in-memory nesne döndürür."""
    tool = build_rest_connector_wrapped
    result = _invoke(
        tool, llm_model,
        "build_rest_connector tool'unu TEK çağrı ile çağır. "
        "config = {\"kind\": \"rest_api\", \"name\": \"api_demo\", \"params\": {}}.",
    )
    ...
    assert "ok" in s or "failed" in s or "error" in s, (
        f"build_rest_connector beklenen tool execution çıktısı üretmedi: {s[:300]}"
    )
