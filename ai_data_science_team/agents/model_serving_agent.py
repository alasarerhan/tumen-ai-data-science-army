...
def load_model(
    ...
    if is_mlflow:
        try:
            import mlflow  # type: ignore  # noqa: E402, F401
    ...
    # ---- Local file ---------------------------------------------------------
        ...
        try:
            if ext in (".joblib",):
                import joblib  # type: ignore  # noqa: E402, F401
...
NODE_TYPE = "deploy.serve"  # was G3_NODE_TYPE before Phase 8 rename

# Backward-compat aliases — tests use spec-prefixed names (Phase 8 rename target).
G3Agent = ModelServingAgent
make_g3_model_serving_agent = make_model_serving_agent
G3_AGENT_NAME = AGENT_NAME
G3_NODE_TYPE = NODE_TYPE
G3_TOOLS = SERVING_TOOLS
