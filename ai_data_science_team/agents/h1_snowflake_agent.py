...
tool layer (``ai_data_science_team.tools.h1_snowflake``) with
LangChain ``@tool`` decorators and exposes the standard
``make_h1_snowflake_agent`` factory + ``H1Agent`` OO
...
H1_TOOLS = [
    h1_build_snowflake_connector_wrapped,
]
...
class H1Agent(BaseAgent):
    ...
    def _make_compiled_graph(self):
        self.response = None
        return make_h1_snowflake_agent(**self._params)
