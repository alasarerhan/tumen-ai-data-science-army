...
Phase-5 agent wrapper for spec H3.  Wraps the deterministic
tool layer (``ai_data_science_team.tools.tableau``) with
LangChain ``@tool`` decorators and exposes the standard
``make_tableau_agent`` factory + ``H3Agent`` OO
wrapper, following the same pattern as ABTestingAgent and
PowerAnalysisAgent.

Node type: ``source.tableau``
"""
...
from ai_data_science_team.tools.tableau import (  # noqa: E402, F401
...
def build_tableau_connector_wrapped(config: ConnectorConfig) -> Tuple[str, dict]:
    ...
    """
    logger.info("    * Tool: h3_build_tableau_connector")
    kwargs = {"config": config}
    try:
        result = build_tableau_connector(**kwargs)
    except Exception as exc:
        return f"Tool h3_build_tableau_connector failed: {exc}", {
            "build_tableau_connector": kwargs,
            "args": kwargs,
            "result": None,
            "content": f"error: {exc}",
        }
    content = "h3_build_tableau_connector: ok"
    return content, {
        "build_tableau_connector": kwargs,
        "args": kwargs,
        "result": result,
...
TABLEAU_SOURCE_TOOLS = [
    build_tableau_connector_wrapped,
]
...
def make_tableau_agent(
    ...
    """Build the LangGraph StateGraph for the H3 agent."""
    ...
    def run_react_agent(state: GraphState):
        logger.info("    * RUN REACT AGENT FOR H3")
        base = state.get("messages") or [("user", state.get("user_instructions"))]
        messages = [
            (
                "system",
                "You are the H3 agent. Use the available tools to complete the user's request.",
            )
        ] + list(base)
    ...
    def post_process(state: GraphState):
        logger.info("    * POST-PROCESSING H3 RESULTS")
...
class TableauSourceAgent(BaseAgent):
    """OO wrapper for the H3 agent (node type ``source.tableau``)."""
    ...
    def _make_compiled_graph(self):
        self.response = None
        return make_tableau_agent(**self._params)
