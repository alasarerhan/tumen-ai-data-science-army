"""
Tests for ``ai_data_science_team.agents.bigquery_agent`` (H2 layer).
...
Phase-6 integration tests covering:
  ...
  3. ``make_bigquery_agent`` factory returns a compiled graph with the
     expected node wiring
  4. ``BigQuerySourceAgent`` (BaseAgent subclass) constructs, exposes accessors,
...
class _StubModel:
    ...
    ``make_bigquery_agent`` calls ``create_agent(model, ...)`` to bind tools;
...
def _agent_with_no_op_react(monkeypatch) -> BigQuerySourceAgent:
    ...
    return BigQuerySourceAgent(model=_StubModel())
...
class TestModuleSurface:
    def test_constants(self):
        ...
        assert AGENT_NAME == "bigquery_agent"
        assert NODE_TYPE == "source.bigquery"

    def test_tool_count_matches_registry(self):
        assert len(BIGQUERY_SOURCE_TOOLS) >= 1
    ...
    def test_tool_names_match_registry(self):
        ...
        wrapper_names = {t.name for t in BIGQUERY_SOURCE_TOOLS}
    ...
    def test_all_individual_tools_exported(self):
        ...
        mod = sys.modules["ai_data_science_team.agents.bigquery_agent"]
        wrapper_names = ["build_bigquery_connector_wrapped"]
...
class TestBuildBigqueryConnector:
    def test_direct_call_returns_tuple(self):
        """Call build_bigquery_connector via its @tool wrapper (LLM-free)."""
        from ai_data_science_team.agents.bigquery_agent import build_bigquery_connector_wrapped
        kwargs = {
            "config": None,
        }
        try:
            result = build_bigquery_connector_wrapped.invoke(kwargs)
...
class TestMakeBigQuerySourceAgent:
    def test_factory_returns_compiled_graph(self):
        app = make_bigquery_agent(model=_StubModel())
    ...
    def test_factory_with_checkpointer(self):
        ...
        app = make_bigquery_agent(model=_StubModel(), checkpointer=InMemorySaver())
...
class TestBigQuerySourceAgent:
    def test_init_compiles_graph(self):
        agent = BigQuerySourceAgent(model=_StubModel())
