...
def upgrade() -> None:
    op.create_table(
        ...
        sa.Column(
            "approval_state", sa.String(length=50), nullable=False, server_default="not_reviewed"
        ),
        sa.Column(
            "deployment_state", sa.String(length=50), nullable=False, server_default="not_deployed"
        ),
        sa.Column(
            "monitoring_status",
            sa.String(length=50),
        ...
        sa.Column(
            "performance_status", sa.String(length=50), nullable=False, server_default="unknown"
        ),
        ...
        sa.UniqueConstraint(
            "workspace_id", "model_name", "version", name="uq_model_registry_workspace_name_version"
        ),
    )
    op.create_index(
    ...
    op.create_index(
        "ix_model_monitor_workspace_created",
        "model_monitor_snapshots",
        ["workspace_id", "created_at"],
    )
    op.create_index(
        "ix_model_monitor_model_status", "model_monitor_snapshots", ["model_id", "status"]
    )
    ...
    op.create_table(
        ...
        sa.ForeignKeyConstraint(["model_id"], ["model_registry_entries.id"], ondelete="CASCADE"),
        sa.ForeignKeyConstraint(
            ["rollback_model_id"], ["model_registry_entries.id"], ondelete="SET NULL"
        ),
    ...
    op.create_index(
        "ix_model_deployments_model_created", "model_deployment_records", ["model_id", "created_at"]
    )
...
def downgrade() -> None:
    ...
    op.drop_index(
        "ix_model_deployments_workspace_env_status", table_name="model_deployment_records"
    )
