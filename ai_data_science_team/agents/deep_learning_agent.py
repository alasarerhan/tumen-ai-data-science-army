...
tool layer (``ai_data_science_team.tools.deep_learning``) with
LangChain ``@tool`` decorators and exposes the standard
``make_deep_learning_agent`` factory + ``E3Agent`` OO
...
AGENT_NAME = "deep_learning_agent"
...
@tool(response_format="content_and_artifact")
def train_mlp_classifier_wrapped(X: np.ndarray, y: np.ndarray) -> Tuple[str, dict]:
...
@tool(response_format="content_and_artifact")
def train_lstm_forecaster_wrapped(X: np.ndarray, y: np.ndarray) -> Tuple[str, dict]:
...
class DeepLearningAgent(BaseAgent):
    ...
    def _make_compiled_graph(self):
        self.response = None
        return make_deep_learning_agent(**self._params)
