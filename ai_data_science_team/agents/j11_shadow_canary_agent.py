...
tool layer (``ai_data_science_team.tools.j11_shadow_canary``) with
LangChain ``@tool`` decorators and exposes the standard
``make_j11_shadow_canary_agent`` factory + ``J11Agent`` OO
...
from ai_data_science_team.tools.j11_shadow_canary import (
    Deployment,
    DeploymentStore,
    LiveSample,
    RollbackPolicy,
...
J11_TOOLS = [
    j11_start_deployment_wrapped,
    j11_record_live_sample_wrapped,
    j11_evaluate_rollback_wrapped,
    j11_mark_status_wrapped,
    j11_summarise_deployment_wrapped,
    j11_list_deployments_wrapped,
]
...
def make_j11_shadow_canary_agent(
    model: Any,
...
class J11Agent(BaseAgent):
    ...
    def _make_compiled_graph(self):
        self.response = None
        return make_j11_shadow_canary_agent(**self._params)
