"""GERÇEK model-driven powerbi_agent tool doğrulaması (PM kararı: stub test yok).
...
def test_build_powerbi_connector_real(llm_or_skip, llm_model):
    ...
    assert "ok" in s or "failed" in s or "error" in s, (
        f"build_powerbi_connector beklenen tool execution çıktısı üretmedi: {s[:300]}"
    )
