"""
Tests for ``ai_data_science_team.agents.h3_tableau_agent`` (H3 layer).
...
Phase-6 integration tests covering:
  ...
  3. ``make_h3_tableau_agent`` factory returns a compiled graph with the
     expected node wiring
  4. ``H3Agent`` (BaseAgent subclass) constructs, exposes accessors,
...
from ai_data_science_team.agents.h3_tableau_agent import (
    build_tableau_connector,
    H3Agent,
    make_h3_tableau_agent,
    AGENT_NAME,
    NODE_TYPE,
    H3_TOOLS,
)
...
class _StubModel:
    ...
    ``make_h3_tableau_agent`` calls ``create_agent(model, ...)`` to bind tools;
...
def _agent_with_no_op_react(monkeypatch) -> H3Agent:
    ...
    return H3Agent(model=_StubModel())
...
class TestModuleSurface:
    ...
    def test_tool_names_match_registry(self):
        ...
        wrapper_names = {t.name for t in H3_TOOLS}
    ...
    def test_all_individual_tools_exported(self):
        ...
        import sys
        mod = sys.modules["ai_data_science_team.agents.h3_tableau_agent"]
        wrapper_names = ["h3_build_tableau_connector_wrapped"]
...
class TestBuildTableauConnector:
    def test_direct_call_returns_tuple(self):
        """Call build_tableau_connector via its @tool wrapper (LLM-free)."""
        from ai_data_science_team.agents.h3_tableau_agent import h3_build_tableau_connector_wrapped
        kwargs = {
            "config": None,
        }
        try:
            result = h3_build_tableau_connector_wrapped.invoke(kwargs)
...
class TestMakeH3Agent:
    def test_factory_returns_compiled_graph(self):
        from langgraph.graph import StateGraph
        app = make_h3_tableau_agent(model=_StubModel())
    ...
    def test_factory_with_checkpointer(self):
        ...
        app = make_h3_tableau_agent(model=_StubModel(), checkpointer=InMemorySaver())
...
class TestH3Agent:
    def test_init_compiles_graph(self):
        agent = H3Agent(model=_StubModel())
