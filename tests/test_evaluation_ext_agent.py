"""evaluation_ext_agent modül yüzeyi + fabrika gerçek testleri (LLM-free).

Bu dosya yalnızca modülün export ettiği sabit isimleri (AGENT_NAME, NODE_TYPE,
EVALUATION_EXT_TOOLS), tool wrapper sınıflarının LangChain ``StructuredTool`` sözleşmesine
uyumluluğunu, ve ``make_evaluation_ext_agent`` fabrikasının beklenen LangGraph node'larını
...
def test_constants():
    """AGENT_NAME lowercase spec_id + '_agent'; NODE_TYPE 'model.evaluate.ext'."""
...
EXPECTED_WRAPPERS = [
    ...
    "evaluate_segments_wrapped",
]


def test_all_individual_tools_exported():
    """3 wrapper'ın hepsi modülden erişilebilir ve StructuredTool sözleşmesine sahip."""
...
# ---------------------------------------------------------------------------
# 3. make_evaluation_ext_agent fabrika wiring (LLM-free, no monkey-patch)
# ---------------------------------------------------------------------------


class _CallableModel:
    """``make_evaluation_ext_agent`` yalnızca ``model`` argümanının callability'sini
    kontrol eder; bu stub, gerçek bir LLM çağrısı yapmaz. Aşağıdaki
    ``make_evaluation_ext_agent`` testleri yalnızca graf düğümlerinin varlığını kontrol eder,
...
def test_make_evaluation_ext_agent_builds_graph():
    """make_evaluation_ext_agent bir LangGraph StateGraph döndürmeli ve en azından
    reakt-agent + post_process node'larını içermeli."""
    graph = make_evaluation_ext_agent(model=_CallableModel())
    ...
    assert len(node_ids) >= 1, f"make_evaluation_ext_agent boş graph döndürdü: {node_ids}"
