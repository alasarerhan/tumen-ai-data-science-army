...
> **Öncelik:** P3 · **Faz:** 4 · **Kapsam:** perturbation/edge-case

## 1. Amaç & Kullanıcı Hikâyeleri

Perturbation/edge-case testleri; gürültü ekleme, feature maskeleme, sentetik adverserial örnekler.

**Kabul senaryoları:**

1. Perturbation: gürültü, feature maskeleme, sentetik edge case.
2. Sonuç matrisi: model × senaryo performansı.

## 2. Backend Tasarımı

**Agent/Servis:** `ai_data_science_team/agents/f5_agent.py` — `langgraph` react-agent; deterministic çekirdek aynı paketin `tools/` altında.

**Node tipi:**
```json
{
  "type": "model.robustness",
  "config": {
...
**Veri modeli:**
- `f5_runs (run_id, status, params_json, result_json, created_at)`
- `f5_artifacts (artifact_ref, run_id, kind, payload)`
...
**Konum:** Model detayında 'Robustness' sekmesi
...
- İlgili node tipi `model.robustness` platform-api-app'te handler'a bağlı.
