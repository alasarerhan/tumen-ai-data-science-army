...
Phase-5 agent wrapper for spec B3.  Wraps the deterministic
tool layer (``ai_data_science_team.tools.schema``) with
LangChain ``@tool`` decorators and exposes the standard
``make_schema_agent`` factory + ``B3Agent`` OO
wrapper, following the same pattern as ABTestingAgent and
PowerAnalysisAgent.

Node type: ``schema.infer``
"""
...
from ai_data_science_team.tools.schema import (  # noqa: E402, F401
    MappingResult,
    Schema,
    build_mapping,
    infer_column_type,
...
def infer_column_type_wrapped(series: pd.Series) -> Tuple[str, dict]:
...
@tool(response_format="content_and_artifact")
def infer_schema_wrapped(df: pd.DataFrame) -> Tuple[str, dict]:
    ...
    Returns a (content, artifact) tuple per the react-agent contract.
    """
    logger.info("    * Tool: b3_infer_schema")
    kwargs = {"df": df}
    try:
...
@tool(response_format="content_and_artifact")
def build_mapping_wrapped(source: Schema, target: Schema) -> Tuple[str, dict]:
...
@tool(response_format="content_and_artifact")
def mapping_summary_wrapped(mapping: MappingResult) -> Tuple[str, dict]:
...
def make_schema_agent(
    ...
    """Build the LangGraph StateGraph for the B3 agent."""
    ...
    def run_react_agent(state: GraphState):
        ...
        messages = [
            (
                "system",
                "You are the B3 agent. Use the available tools to complete the user's request.",
            )
        ] + list(base)
...
class SchemaInferenceAgent(BaseAgent):
    """OO wrapper for the B3 agent (node type ``schema.infer``)."""
    ...
    def _make_compiled_graph(self):
        self.response = None
        return make_schema_agent(**self._params)
