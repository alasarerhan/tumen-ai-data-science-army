...
def detect_mime_from_magic_bytes(file_bytes: bytes) -> tuple[str, str]:
    ...
    try:
        ...
        if (
            text.replace("\n", "").replace("\r", "").replace(",", "").replace(" ", "").isalnum()
            or "," in text
        ):
            return (".csv", "text/csv")
    except Exception:
...
def validate_file_extension(filename: str) -> str:
    ...
    if ext in DANGEROUS_EXTENSIONS:
        raise HTTPException(
            status_code=400, detail=f"File type '{ext}' is not allowed for security reasons"
        )
...
def validate_mime_type(content_type: str | None, detected_mime: str) -> str:
    ...
    if detected_mime not in ALLOWED_MIME_TYPES and detected_mime != "application/octet-stream":
        raise HTTPException(
            status_code=400, detail=f"File content type '{detected_mime}' is not allowed"
        )
        ...
        with zipfile.ZipFile(io.BytesIO(file_bytes)) as zf:
            ...
            for info in zf.infolist():
                if info.filename.startswith("/") or ".." in info.filename:
                    raise HTTPException(
                        status_code=400, detail="ZIP contains path traversal attempt"
                    )
                ...
                if (info.external_attr >> 28) == 0xA:
                    raise HTTPException(
                        status_code=400, detail="ZIP contains symlinks which are not allowed"
                    )

                ext = Path(info.filename).suffix.lower()
                if ext in DANGEROUS_EXTENSIONS:
                    raise HTTPException(
                        status_code=400, detail=f"ZIP contains disallowed file type: {ext}"
                    )
...
def sanitize_svg_content(file_bytes: bytes) -> bytes:
    ...
    content = re.sub(r'<use[^>]*href\s*=\s*["\']https?:', "<use ", content, flags=re.IGNORECASE)
