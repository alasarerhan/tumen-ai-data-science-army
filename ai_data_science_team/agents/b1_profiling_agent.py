...
tool layer (``ai_data_science_team.tools.b1_profiling``) with
LangChain ``@tool`` decorators and exposes the standard
``make_b1_profiling_agent`` factory + ``B1Agent`` OO
...
B1_TOOLS = [
    b1_profile_column_wrapped,
    b1_profile_dataframe_wrapped,
]
...
class B1Agent(BaseAgent):
    ...
    def _make_compiled_graph(self):
        self.response = None
        return make_b1_profiling_agent(**self._params)
