...
Phase-5 agent wrapper for spec H2.  Wraps the deterministic
tool layer (``ai_data_science_team.tools.bigquery``) with
LangChain ``@tool`` decorators and exposes the standard
``make_bigquery_agent`` factory + ``H2Agent`` OO
wrapper, following the same pattern as ABTestingAgent and
PowerAnalysisAgent.

Node type: ``source.bigquery``
"""
...
from ai_data_science_team.tools.bigquery import (  # noqa: E402, F401
...
def build_bigquery_connector_wrapped(config: ConnectorConfig) -> Tuple[str, dict]:
    ...
    """
    logger.info("    * Tool: h2_build_bigquery_connector")
    kwargs = {"config": config}
    try:
        result = build_bigquery_connector(**kwargs)
    except Exception as exc:
        return f"Tool h2_build_bigquery_connector failed: {exc}", {
            "build_bigquery_connector": kwargs,
            "args": kwargs,
            "result": None,
            "content": f"error: {exc}",
        }
    content = "h2_build_bigquery_connector: ok"
    return content, {
        "build_bigquery_connector": kwargs,
        "args": kwargs,
        "result": result,
...
def make_bigquery_agent(
    ...
    """Build the LangGraph StateGraph for the H2 agent."""
    ...
    def run_react_agent(state: GraphState):
        logger.info("    * RUN REACT AGENT FOR H2")
        base = state.get("messages") or [("user", state.get("user_instructions"))]
        messages = [
            (
                "system",
                "You are the H2 agent. Use the available tools to complete the user's request.",
            )
        ] + list(base)
    ...
    def post_process(state: GraphState):
        logger.info("    * POST-PROCESSING H2 RESULTS")
...
class BigQuerySourceAgent(BaseAgent):
    """OO wrapper for the H2 agent (node type ``source.bigquery``)."""
    ...
    def _make_compiled_graph(self):
        self.response = None
        return make_bigquery_agent(**self._params)
