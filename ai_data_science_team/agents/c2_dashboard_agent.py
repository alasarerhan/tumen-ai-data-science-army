...
tool layer (``ai_data_science_team.tools.c2_dashboard``) with
LangChain ``@tool`` decorators and exposes the standard
``make_c2_dashboard_agent`` factory + ``C2Agent`` OO
...
C2_TOOLS = [
    c2_add_panel_wrapped,
    c2_validate_layout_wrapped,
    c2_make_share_token_wrapped,
    c2_render_snapshot_wrapped,
    c2_make_dashboard_wrapped,
]
...
class C2Agent(BaseAgent):
    ...
    def _make_compiled_graph(self):
        self.response = None
        return make_c2_dashboard_agent(**self._params)
