...
def generate_ydata_report(
    ...
    report_name: str = "ydata_report.html",
    report_directory: str = None,
    minimal: bool = False,
) -> Tuple[str, Dict]:
    """
    Tool: generate_ydata_report
    Description:
        Generate a comprehensive EDA report using ydata-profiling (formerly pandas-profiling).
        This is a modern alternative to Sweetviz with better numpy 2.x compatibility.
    ...
    data_raw : dict
        The raw data injected as a dictionary (converted from a DataFrame).
    title : str, optional
        The title of the report. Default is "YData Profiling Report".
    report_name : str, optional
        The file name to save the report. Default is "ydata_report.html".
    report_directory : str, optional
        The directory where the report should be saved.
        If None, a temporary directory is created and used.
    minimal : bool, optional
        If True, generates a minimal report (faster). Default is False.

    Returns:
    --------
    Tuple[str, Dict]:
        content: A summary message describing the generated report.
        artifact: A dictionary with the report file path and HTML content.
    """
    print("    * Tool: generate_ydata_report")
    ...
    profile = ProfileReport(df, title=title, minimal=minimal, explorative=True)
    ...
    content = (
        f"YData Profiling report generated and saved as '{os.path.abspath(full_report_path)}'. "
        f"{'This was saved in a temporary directory.' if 'tmp' in report_directory else ''}"
    )
    artifact = {
        "report_file": os.path.abspath(full_report_path),
