"""
Tests for ``ai_data_science_team.agents.h7_rest_api_agent`` (H7 layer).
...
Phase-6 integration tests covering:
  ...
  3. ``make_h7_rest_api_agent`` factory returns a compiled graph with the
     expected node wiring
  4. ``H7Agent`` (BaseAgent subclass) constructs, exposes accessors,
...
from ai_data_science_team.agents.h7_rest_api_agent import (
    build_rest_connector,
    H7Agent,
    make_h7_rest_api_agent,
    AGENT_NAME,
    NODE_TYPE,
    H7_TOOLS,
)
...
class _StubModel:
    ...
    ``make_h7_rest_api_agent`` calls ``create_agent(model, ...)`` to bind tools;
...
def _agent_with_no_op_react(monkeypatch) -> H7Agent:
    ...
    return H7Agent(model=_StubModel())
...
class TestModuleSurface:
    def test_constants(self):
        ...
        assert AGENT_NAME == "h7_rest_api_agent"
        assert NODE_TYPE == "source.rest_api"

    def test_tool_count_matches_registry(self):
        assert len(H7_TOOLS) >= 1
    ...
    def test_tool_names_match_registry(self):
        ...
        wrapper_names = {t.name for t in H7_TOOLS}
    ...
    def test_all_individual_tools_exported(self):
        ...
        import sys
        mod = sys.modules["ai_data_science_team.agents.h7_rest_api_agent"]
        wrapper_names = ["h7_build_rest_connector_wrapped"]
...
class TestBuildRestConnector:
    def test_direct_call_returns_tuple(self):
        """Call build_rest_connector via its @tool wrapper (LLM-free)."""
        from ai_data_science_team.agents.h7_rest_api_agent import h7_build_rest_connector_wrapped
        kwargs = {
            "config": None,
        }
        try:
            result = h7_build_rest_connector_wrapped.invoke(kwargs)
...
class TestMakeH7Agent:
    def test_factory_returns_compiled_graph(self):
        from langgraph.graph import StateGraph
        app = make_h7_rest_api_agent(model=_StubModel())
    ...
    def test_factory_with_checkpointer(self):
        ...
        app = make_h7_rest_api_agent(model=_StubModel(), checkpointer=InMemorySaver())
...
class TestH7Agent:
    def test_init_compiles_graph(self):
        agent = H7Agent(model=_StubModel())
