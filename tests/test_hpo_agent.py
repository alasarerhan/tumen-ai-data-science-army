"""hpo_agent modül yüzeyi + fabrika gerçek testleri (LLM-free).

Bu dosya yalnızca modülün export ettiği sabit isimleri (AGENT_NAME, NODE_TYPE,
HPO_TOOLS), tool wrapper sınıflarının LangChain ``StructuredTool`` sözleşmesine
uyumluluğunu, ve ``make_hpo_agent`` fabrikasının beklenen LangGraph node'larını
ürettiğini doğrular. **LLM çağrısı içermez** — bu yüzden hızlıdır ve LLM yokken
koşabilir.

Tool davranış testleri (2 PURE tool) ``tests/llm/test_hpo_agent_tool_calling.py``
altında **gerçek model-driven** olarak yazılmıştır. 1 STATEFUL tool (run_study:
``Callable`` arg alır, Pydantic JSON-serializable değil) API test Faz C'de
...
def test_constants():
    """AGENT_NAME lowercase spec_id + '_agent'; NODE_TYPE 'model.hpo'."""
...
# ---------------------------------------------------------------------------
# 3. make_hpo_agent fabrika wiring (LLM-free, no monkey-patch)
# ---------------------------------------------------------------------------


class _CallableModel:
    """``make_hpo_agent`` yalnızca ``model`` argümanının callability'sini
    kontrol eder; bu stub, gerçek bir LLM çağrısı yapmaz. Aşağıdaki
    ``make_hpo_agent`` testleri yalnızca graf düğümlerinin varlığını kontrol eder,
...
def test_make_hpo_agent_builds_graph():
    """make_hpo_agent bir LangGraph StateGraph döndürmeli ve en azından
    reakt-agent + post_process node'larını içermeli."""
    graph = make_hpo_agent(model=_CallableModel())
    ...
    assert len(node_ids) >= 1, f"make_hpo_agent boş graph döndürdü: {node_ids}"
