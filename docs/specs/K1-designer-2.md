# K1 — Workflow Designer 2.0

> **Öncelik:** P0 · **Faz:** 1 · **Kapsam:** designer cockpit

## 1. Amaç & Kullanıcı Hikâyeleri

Workflow Designer kokpit yenilemesi: Flow Zones, zengin node kartları, inline validasyon, copilot dock.

**Kabul senaryoları:**

1. Flow Zones: node'ları bölgelere gruplama (Ingest/Prep/Train/Evaluate/Deploy).
2. Zengin node kartları: canlı durum, I-O şema özeti, veri önizleme popover'ı.
3. Inline validasyon + copilot dock.

## 2. Backend Tasarımı

**Agent/Servis:** `ai_data_science_team/agents/k1_agent.py` — `langgraph` react-agent; deterministic çekirdek aynı paketin `tools/` altında.
...
**Veri modeli:**
- `k1_runs (run_id, status, params_json, result_json, created_at)`
- `k1_artifacts (artifact_ref, run_id, kind, payload)`
...
**Konum:** Designer canvas (yenileme — flow zones, inline validate)
...
- İlgili node tipi `designer.render` platform-api-app'te handler'a bağlı.
