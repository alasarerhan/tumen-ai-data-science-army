...
tool layer (``ai_data_science_team.tools.kpi``) with
LangChain ``@tool`` decorators and exposes the standard
``make_kpi_agent`` factory + ``C3Agent`` OO
...
@tool(response_format="content_and_artifact")
def compute_schedule_wrapped() -> Tuple[str, dict]:
...
@tool(response_format="content_and_artifact")
def record_period_wrapped(kpi: Mapping[str, Any]) -> Tuple[str, dict]:
...
@tool(response_format="content_and_artifact")
def make_history_wrapped(kpi_id: str) -> Tuple[str, dict]:
...
@tool(response_format="content_and_artifact")
def build_alarm_wrapped(kpi_id: str) -> Tuple[str, dict]:
...
@tool(response_format="content_and_artifact")
def check_alarm_wrapped(rule: AlarmRule) -> Tuple[str, dict]:
...
@tool(response_format="content_and_artifact")
def sparkline_points_wrapped(values: Sequence[float], n: int) -> Tuple[str, dict]:
...
class KPIMetricsAgent(BaseAgent):
    ...
    def _make_compiled_graph(self):
        self.response = None
        return make_kpi_agent(**self._params)
