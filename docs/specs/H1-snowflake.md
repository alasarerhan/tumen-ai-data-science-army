# H1 — Snowflake Connector

> **Öncelik:** P1 · **Faz:** 2 · **Kapsam:** Snowflake key-pair/SSO

## 1. Amaç & Kullanıcı Hikâyeleri

Snowflake bağlantısı: key-pair veya SSO auth, warehouse seçimi, pushdown.

**Kabul senaryoları:**

1. Snowflake: key-pair veya SSO auth.
2. Warehouse seçimi + pushdown (compute kaynağı seçimi).
3. Bağlantı testi → örnek sorgu.

## 2. Backend Tasarımı

**Agent/Servis:** `ai_data_science_team/agents/h1_agent.py` — `langgraph` react-agent; deterministic çekirdek aynı paketin `tools/` altında.

**Node tipi:**
```json
{
  "type": "connector.snowflake",
  "config": {
...
**API endpoint'leri:**
- `POST /connectors/snowflake`
- `GET /connectors/snowflake/{id}`
- `GET /connectors/snowflake`

**Veri modeli:**
- `h1_runs (run_id, status, params_json, result_json, created_at)`
- `h1_artifacts (artifact_ref, run_id, kind, payload)`
...
**Konum:** Connectors galerisinde Snowflake kartı
...
- İlgili node tipi `connector.snowflake` platform-api-app'te handler'a bağlı.
