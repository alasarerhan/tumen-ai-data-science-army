...
Phase-5 agent wrapper for spec F6.  Wraps the deterministic
tool layer (``ai_data_science_team.tools.llm_judge``) with
LangChain ``@tool`` decorators and exposes the standard
``make_llm_judge_agent`` factory + ``F6Agent`` OO
wrapper, following the same pattern as ABTestingAgent and
PowerAnalysisAgent.

Node type: ``model.llm_judge``
"""
...
from ai_data_science_team.tools.llm_judge import (  # noqa: E402, F401
    judge_batch,
    judge_output,
)
...
def judge_output_wrapped(text: str, code: str) -> Tuple[str, dict]:
    ...
    """
    logger.info("    * Tool: f6_judge_output")
    kwargs = {"text": text, "code": code}
    try:
...
@tool(response_format="content_and_artifact")
def judge_batch_wrapped(items: Sequence[Mapping[str, str]]) -> Tuple[str, dict]:
    ...
    Returns a (content, artifact) tuple per the react-agent contract.
    """
    logger.info("    * Tool: f6_judge_batch")
    kwargs = {"items": items}
    try:
...
def make_llm_judge_agent(
    ...
    """Build the LangGraph StateGraph for the F6 agent."""
    ...
    def run_react_agent(state: GraphState):
        logger.info("    * RUN REACT AGENT FOR F6")
        base = state.get("messages") or [("user", state.get("user_instructions"))]
        messages = [
            (
                "system",
                "You are the F6 agent. Use the available tools to complete the user's request.",
            )
        ] + list(base)
    ...
    def post_process(state: GraphState):
        logger.info("    * POST-PROCESSING F6 RESULTS")
...
class LLMJudgeAgent(BaseAgent):
    """OO wrapper for the F6 agent (node type ``model.llm_judge``)."""
    ...
    def _make_compiled_graph(self):
        self.response = None
        return make_llm_judge_agent(**self._params)
