...
class SLAEscalationManager:
    ...
    async def schedule_escalation(
        ...
        await schedule_task(
            ...
            payload={
                "approval_id": approval_id,
                "action": "escalate",
                "escalate_to": sla_config.escalation_path[0]
                if sla_config.escalation_path
                else None,
            },
        )
    ...
    async def handle_timeout(self, approval_id: str) -> None:
        ...
        if approval.status != "pending":
            logger.info(
                "Approval %s is no longer pending (status=%s)", approval_id, approval.status
            )
            return
    ...
    async def _escalate(self, approval: ApprovalRequest) -> None:
        ...
        next_level = (
            approval.escalation_path[approval.escalation_level - 1]
            if approval.escalation_level <= len(approval.escalation_path)
            else "admin"
        )
