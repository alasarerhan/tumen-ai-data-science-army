...
> **Öncelik:** P1 · **Faz:** 2 · **Kapsam:** KPI değişim kök neden

## 1. Amaç & Kullanıcı Hikâyeleri

KPI değişimi sinyali geldiğinde otomatik kök-neden analizi: boyut bazlı katkı ayrıştırması, drill-down, LLM anlatı özeti.

**Kabul senaryoları:**

1. KPI değişimi → otomatik kök neden başlar.
2. Waterfall (boyut katkı dağılımı) + top-N segment listesi + LLM anlatı özeti.
3. Her boyuta tıklayıp alt kırılımları drill-down.

## 2. Backend Tasarımı

**Agent/Servis:** `ai_data_science_team/agents/c4_agent.py` — `langgraph` react-agent; deterministic çekirdek aynı paketin `tools/` altında.

**Node tipi:**
```json
{
  "type": "metric.rootcause",
  "config": {
...
**Veri modeli:**
- `c4_runs (run_id, status, params_json, result_json, created_at)`
- `c4_artifacts (artifact_ref, run_id, kind, payload)`
...
**Konum:** KPI detayında 'Root cause' sekmesi; graf + drill-down
...
- İlgili node tipi `metric.rootcause` platform-api-app'te handler'a bağlı.
