"""rootcause_agent modül yüzeyi + fabrika gerçek testleri (LLM-free).

Bu dosya yalnızca modülün export ettiği sabit isimleri (AGENT_NAME, NODE_TYPE,
ROOT_CAUSE_ANALYSIS_TOOLS), tool wrapper sınıflarının LangChain ``StructuredTool`` sözleşmesine
uyumluluğunu, ve ``make_rootcause_agent`` fabrikasının beklenen LangGraph node'larını
ürettiğini doğrular. **LLM çağrısı içermez** — bu yüzden hızlıdır ve LLM yokken
koşabilir.

Tüm tool'lar stateful (pd.DataFrame, WaterfallResult); tool davranış testleri
...
def test_constants():
    """AGENT_NAME lowercase spec_id + '_agent'; NODE_TYPE 'kpi.root_cause'."""
...
# ---------------------------------------------------------------------------
# 3. make_rootcause_agent fabrika wiring (LLM-free, no monkey-patch)
# ---------------------------------------------------------------------------


class _CallableModel:
    """``make_rootcause_agent`` yalnızca ``model`` argümanının callability'sini
    kontrol eder; bu stub, gerçek bir LLM çağrısı yapmaz. Aşağıdaki
    ``make_rootcause_agent`` testleri yalnızca graf düğümlerinin varlığını kontrol eder,
...
def test_make_rootcause_agent_builds_graph():
    """make_rootcause_agent bir LangGraph StateGraph döndürmeli ve en azından
    reakt-agent + post_process node'larını içermeli."""
    graph = make_rootcause_agent(model=_CallableModel())
    ...
    assert len(node_ids) >= 1, f"make_rootcause_agent boş graph döndürdü: {node_ids}"
