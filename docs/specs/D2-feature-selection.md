# D2 — Feature Selection + Leakage

> **Öncelik:** P1 · **Faz:** 2 · **Kapsam:** feature seçim + leakage taraması

## 1. Amaç & Kullanıcı Hikâyeleri

Filter/wrapper/embedded feature seçim algoritmaları + target-leakage taraması + multicollinearity raporu.

**Kabul senaryoları:**

1. Filter/wrapper/embedded feature seçim.
2. Target leakage taraması şüpheli kolonları işaretler.
3. VIF + korelasyon ısı haritası ile multicollinearity raporu.

## 2. Backend Tasarımı

**Agent/Servis:** `ai_data_science_team/agents/d2_agent.py` — `langgraph` react-agent; deterministic çekirdek aynı paketin `tools/` altında.
...
- `GET /features/select/{id}`
- `GET /features/select`

**Veri modeli:**
- `d2_runs (run_id, status, params_json, result_json, created_at)`
- `d2_artifacts (artifact_ref, run_id, kind, payload)`
...
**Konum:** Designer'da 'feature select' node config paneli
...
- İlgili node tipi `feature.select` platform-api-app'te handler'a bağlı.
