...
def load_file(file_path: str) -> Tuple[str, Dict]:
    ...
    return f"Returned the following data frame from this file: {file_path}", auto_load_file(
        file_path
    ).to_dict()
...
def list_directory_contents(
    ...
    for item in items:
        ...
        artifact.append(
            {"filename": item, "type": "directory" if os.path.isdir(item_path) else "file"}
        )
    ...
    def recurse(path: str, indent_level: int = 0):
        ...
        for item in items:
            ...
            if os.path.isdir(full_path):
                ...
                lines.append(f"{prefix}- {item}")
                records.append(
                    {"type": "file", "name": item, "parent_path": path, "absolute_path": full_path}
                )
