...
tool layer (``ai_data_science_team.tools.a5_causal``) with
LangChain ``@tool`` decorators and exposes the standard
``make_a5_causal_agent`` factory + ``A5Agent`` OO
...
A5_TOOLS = [
    a5_did_lift_wrapped,
    a5_adj_lift_wrapped,
    a5_check_propensity_overlap_wrapped,
    a5_e_value_wrapped,
]
...
class A5Agent(BaseAgent):
    ...
    def _make_compiled_graph(self):
        self.response = None
        return make_a5_causal_agent(**self._params)
