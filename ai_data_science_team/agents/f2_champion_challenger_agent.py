...
Phase-5 agent wrapper for spec F2.  Wraps the deterministic
tool layer (``ai_data_science_team.tools.f2_champion_challenger``) with
LangChain ``@tool`` decorators and exposes the standard
``make_f2_champion_challenger_agent`` factory + ``F2Agent`` OO
...
F2_TOOLS = [
    f2_mcnemar_test_wrapped,
    f2_wilcoxon_signed_rank_wrapped,
    f2_auc_with_delong_ci_wrapped,
    f2_delong_pvalue_wrapped,
    f2_compare_models_wrapped,
]
...
def make_f2_champion_challenger_agent(
    model: Any,
    ...
    """Build the LangGraph StateGraph for the F2 agent."""
...
class F2Agent(BaseAgent):
    """OO wrapper for the F2 agent (node type ``model.champion_challenger``)."""
    ...
    def _make_compiled_graph(self):
        self.response = None
        return make_f2_champion_challenger_agent(**self._params)
