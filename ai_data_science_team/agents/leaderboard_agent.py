...
tool layer (``ai_data_science_team.tools.leaderboard``) with
LangChain ``@tool`` decorators and exposes the standard
``make_leaderboard_agent`` factory + ``I3Agent`` OO
...
from ai_data_science_team.tools.leaderboard import (  # noqa: E402, F401
    ExperimentStore,
    leaderboard,
...
def record_run_wrapped(store: ExperimentStore) -> Tuple[str, dict]:
...
@tool(response_format="content_and_artifact")
def leaderboard_wrapped(
    store: ExperimentStore, experiment_id: str, primary_metric: str
) -> Tuple[str, dict]:
...
@tool(response_format="content_and_artifact")
def summarise_metrics_wrapped(
    store: ExperimentStore, experiment_id: str, metric: str
) -> Tuple[str, dict]:
    ...
    try:
        ...
        return f"Tool i3_summarise_metrics failed: {exc}", {
            "summarise_metrics": kwargs,
            "args": kwargs,
            "result": None,
            "content": f"error: {exc}",
        }
    content = "i3_summarise_metrics: ok"
    return content, {
        "summarise_metrics": kwargs,
        "args": kwargs,
        "result": result,
...
@tool(response_format="content_and_artifact")
def parallel_coordinates_payload_wrapped(
    store: ExperimentStore, experiment_id: str, metric_columns: Sequence[str]
) -> Tuple[str, dict]:
    ...
    try:
        ...
        return f"Tool i3_parallel_coordinates_payload failed: {exc}", {
            "parallel_coordinates_payload": kwargs,
            "args": kwargs,
            "result": None,
            "content": f"error: {exc}",
        }
    content = "i3_parallel_coordinates_payload: ok"
    return content, {
        "parallel_coordinates_payload": kwargs,
        "args": kwargs,
        "result": result,
...
def make_leaderboard_agent(
    ...
    react_agent = create_agent(
        model,
        tools=EXPERIMENT_TRACKER_TOOLS,
    ...
    def run_react_agent(state: GraphState):
        logger.info("    * RUN REACT AGENT FOR I3")
        base = state.get("messages") or [("user", state.get("user_instructions"))]
        messages = [
            (
                "system",
                "You are the I3 agent. Use the available tools to complete the user's request.",
            )
        ] + list(base)
    ...
    def post_process(state: GraphState):
        logger.info("    * POST-PROCESSING I3 RESULTS")
...
class ExperimentTrackerAgent(BaseAgent):
    ...
    def _make_compiled_graph(self):
        self.response = None
        return make_leaderboard_agent(**self._params)
