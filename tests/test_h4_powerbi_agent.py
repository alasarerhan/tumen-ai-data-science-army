"""
Tests for ``ai_data_science_team.agents.h4_powerbi_agent`` (H4 layer).
...
Phase-6 integration tests covering:
  ...
  3. ``make_h4_powerbi_agent`` factory returns a compiled graph with the
     expected node wiring
  4. ``H4Agent`` (BaseAgent subclass) constructs, exposes accessors,
...
from ai_data_science_team.agents.h4_powerbi_agent import (
    build_powerbi_connector,
    H4Agent,
    make_h4_powerbi_agent,
    AGENT_NAME,
    NODE_TYPE,
    H4_TOOLS,
)
...
class _StubModel:
    ...
    ``make_h4_powerbi_agent`` calls ``create_agent(model, ...)`` to bind tools;
...
def _agent_with_no_op_react(monkeypatch) -> H4Agent:
    ...
    return H4Agent(model=_StubModel())
...
class TestModuleSurface:
    ...
    def test_tool_names_match_registry(self):
        ...
        wrapper_names = {t.name for t in H4_TOOLS}
    ...
    def test_all_individual_tools_exported(self):
        ...
        import sys
        mod = sys.modules["ai_data_science_team.agents.h4_powerbi_agent"]
        wrapper_names = ["h4_build_powerbi_connector_wrapped"]
...
class TestBuildPowerbiConnector:
    def test_direct_call_returns_tuple(self):
        """Call build_powerbi_connector via its @tool wrapper (LLM-free)."""
        from ai_data_science_team.agents.h4_powerbi_agent import h4_build_powerbi_connector_wrapped
        kwargs = {
            "config": None,
        }
        try:
            result = h4_build_powerbi_connector_wrapped.invoke(kwargs)
...
class TestMakeH4Agent:
    def test_factory_returns_compiled_graph(self):
        from langgraph.graph import StateGraph
        app = make_h4_powerbi_agent(model=_StubModel())
    ...
    def test_factory_with_checkpointer(self):
        ...
        app = make_h4_powerbi_agent(model=_StubModel(), checkpointer=InMemorySaver())
...
class TestH4Agent:
    def test_init_compiles_graph(self):
        agent = H4Agent(model=_StubModel())
