"""GERÇEK model-driven snowflake_agent tool doğrulaması (PM kararı: stub test yok).
...
def test_build_snowflake_connector_real(llm_or_skip, llm_model):
    """build_snowflake_connector Pydantic ConnectorConfig alır; in-memory nesne döndürür."""
    tool = build_snowflake_connector_wrapped
    result = _invoke(
        tool, llm_model,
        "build_snowflake_connector tool'unu TEK çağrı ile çağır. "
        "config = {\"kind\": \"snowflake\", \"name\": \"sf_demo\", \"params\": {}}.",
    )
    ...
    assert "ok" in s or "failed" in s or "error" in s, (
        f"build_snowflake_connector beklenen tool execution çıktısı üretmedi: {s[:300]}"
    )
