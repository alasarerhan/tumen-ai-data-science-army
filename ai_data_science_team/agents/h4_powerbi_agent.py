...
tool layer (``ai_data_science_team.tools.h4_powerbi``) with
LangChain ``@tool`` decorators and exposes the standard
``make_h4_powerbi_agent`` factory + ``H4Agent`` OO
...
from ai_data_science_team.tools.h1_snowflake import ConnectorConfig

from ai_data_science_team.tools.h4_powerbi import (
    PowerBIConnector,
...
def h4_build_powerbi_connector_wrapped(config: ConnectorConfig) -> Tuple[str, {
    "name": str,
    "args": dict,
    "result": PowerBIConnector,
    "content": str,
}]:
...
H4_TOOLS = [
    h4_build_powerbi_connector_wrapped,
]
...
class H4Agent(BaseAgent):
    ...
    def _make_compiled_graph(self):
        self.response = None
        return make_h4_powerbi_agent(**self._params)
