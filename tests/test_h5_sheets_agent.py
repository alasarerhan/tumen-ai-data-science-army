"""
Tests for ``ai_data_science_team.agents.h5_sheets_agent`` (H5 layer).
...
Phase-6 integration tests covering:
  ...
  3. ``make_h5_sheets_agent`` factory returns a compiled graph with the
     expected node wiring
  4. ``H5Agent`` (BaseAgent subclass) constructs, exposes accessors,
...
from ai_data_science_team.agents.h5_sheets_agent import (
    build_sheets_connector,
    H5Agent,
    make_h5_sheets_agent,
    AGENT_NAME,
    NODE_TYPE,
    H5_TOOLS,
)
...
class _StubModel:
    ...
    ``make_h5_sheets_agent`` calls ``create_agent(model, ...)`` to bind tools;
...
def _agent_with_no_op_react(monkeypatch) -> H5Agent:
    ...
    return H5Agent(model=_StubModel())
...
class TestModuleSurface:
    def test_constants(self):
        ...
        assert AGENT_NAME == "h5_sheets_agent"
        assert NODE_TYPE == "source.sheets"

    def test_tool_count_matches_registry(self):
        assert len(H5_TOOLS) >= 1
    ...
    def test_tool_names_match_registry(self):
        ...
        wrapper_names = {t.name for t in H5_TOOLS}
    ...
    def test_all_individual_tools_exported(self):
        ...
        import sys
        mod = sys.modules["ai_data_science_team.agents.h5_sheets_agent"]
        wrapper_names = ["h5_build_sheets_connector_wrapped"]
...
class TestBuildSheetsConnector:
    def test_direct_call_returns_tuple(self):
        """Call build_sheets_connector via its @tool wrapper (LLM-free)."""
        from ai_data_science_team.agents.h5_sheets_agent import h5_build_sheets_connector_wrapped
        kwargs = {
            "config": None,
        }
        try:
            result = h5_build_sheets_connector_wrapped.invoke(kwargs)
...
class TestMakeH5Agent:
    def test_factory_returns_compiled_graph(self):
        from langgraph.graph import StateGraph
        app = make_h5_sheets_agent(model=_StubModel())
    ...
    def test_factory_with_checkpointer(self):
        ...
        app = make_h5_sheets_agent(model=_StubModel(), checkpointer=InMemorySaver())
...
class TestH5Agent:
    def test_init_compiles_graph(self):
        agent = H5Agent(model=_StubModel())
