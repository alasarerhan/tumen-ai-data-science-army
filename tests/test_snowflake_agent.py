"""
Tests for ``ai_data_science_team.agents.snowflake_agent`` (H1 layer).
...
Phase-6 integration tests covering:
  ...
  3. ``make_snowflake_agent`` factory returns a compiled graph with the
     expected node wiring
  4. ``SnowflakeSourceAgent`` (BaseAgent subclass) constructs, exposes accessors,
...
class _StubModel:
    ...
    ``make_snowflake_agent`` calls ``create_agent(model, ...)`` to bind tools;
...
def _agent_with_no_op_react(monkeypatch) -> SnowflakeSourceAgent:
    ...
    return SnowflakeSourceAgent(model=_StubModel())
...
class TestModuleSurface:
    def test_constants(self):
        ...
        assert AGENT_NAME == "snowflake_agent"
        assert NODE_TYPE == "source.snowflake"

    def test_tool_count_matches_registry(self):
        assert len(SNOWFLAKE_SOURCE_TOOLS) >= 1
    ...
    def test_tool_names_match_registry(self):
        ...
        wrapper_names = {t.name for t in SNOWFLAKE_SOURCE_TOOLS}
    ...
    def test_all_individual_tools_exported(self):
        ...
        mod = sys.modules["ai_data_science_team.agents.snowflake_agent"]
        wrapper_names = ["build_snowflake_connector_wrapped"]
...
class TestBuildSnowflakeConnector:
    def test_direct_call_returns_tuple(self):
        """Call build_snowflake_connector via its @tool wrapper (LLM-free)."""
        from ai_data_science_team.agents.snowflake_agent import build_snowflake_connector_wrapped
        kwargs = {
            "config": None,
        }
        try:
            result = build_snowflake_connector_wrapped.invoke(kwargs)
...
class TestMakeSnowflakeSourceAgent:
    def test_factory_returns_compiled_graph(self):
        app = make_snowflake_agent(model=_StubModel())
    ...
    def test_factory_with_checkpointer(self):
        ...
        app = make_snowflake_agent(model=_StubModel(), checkpointer=InMemorySaver())
...
class TestSnowflakeSourceAgent:
    def test_init_compiles_graph(self):
        agent = SnowflakeSourceAgent(model=_StubModel())
