...
tool layer (``ai_data_science_team.tools.champion_challenger``) with
LangChain ``@tool`` decorators and exposes the standard
``make_champion_challenger_agent`` factory + ``F2Agent`` OO
...
AGENT_NAME = "champion_challenger_agent"
...
def make_champion_challenger_agent(
    model: Any,
    ...
    react_agent = create_agent(
        model,
        tools=CHAMPION_CHALLENGER_TOOLS,
...
class ChampionChallengerAgent(BaseAgent):
    ...
    def _make_compiled_graph(self):
        self.response = None
        return make_champion_challenger_agent(**self._params)
