"""
Tests for ``ai_data_science_team.agents.tableau_agent`` (H3 layer).
...
Phase-6 integration tests covering:
  ...
  3. ``make_tableau_agent`` factory returns a compiled graph with the
     expected node wiring
  4. ``TableauSourceAgent`` (BaseAgent subclass) constructs, exposes accessors,
...
class _StubModel:
    ...
    ``make_tableau_agent`` calls ``create_agent(model, ...)`` to bind tools;
...
def _agent_with_no_op_react(monkeypatch) -> TableauSourceAgent:
    ...
    return TableauSourceAgent(model=_StubModel())
...
class TestBuildTableauConnector:
    def test_direct_call_returns_tuple(self):
        ...
        from ai_data_science_team.agents.tableau_agent import build_tableau_connector_wrapped
        kwargs = {
            "config": None,
        }
        try:
            result = build_tableau_connector_wrapped.invoke(kwargs)
...
class TestMakeTableauSourceAgent:
    def test_factory_returns_compiled_graph(self):
        app = make_tableau_agent(model=_StubModel())
    ...
    def test_factory_with_checkpointer(self):
        ...
        app = make_tableau_agent(model=_StubModel(), checkpointer=InMemorySaver())
...
class TestTableauSourceAgent:
    def test_init_compiles_graph(self):
        agent = TableauSourceAgent(model=_StubModel())
