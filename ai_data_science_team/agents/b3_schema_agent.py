...
tool layer (``ai_data_science_team.tools.b3_schema``) with
LangChain ``@tool`` decorators and exposes the standard
``make_b3_schema_agent`` factory + ``B3Agent`` OO
...
B3_TOOLS = [
    b3_infer_column_type_wrapped,
    b3_infer_schema_wrapped,
    b3_build_mapping_wrapped,
    b3_mapping_summary_wrapped,
]
...
class B3Agent(BaseAgent):
    ...
    def _make_compiled_graph(self):
        self.response = None
        return make_b3_schema_agent(**self._params)
