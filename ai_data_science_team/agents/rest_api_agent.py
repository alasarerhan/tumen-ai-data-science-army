...
Phase-5 agent wrapper for spec H7.  Wraps the deterministic
tool layer (``ai_data_science_team.tools.rest_api``) with
LangChain ``@tool`` decorators and exposes the standard
``make_rest_api_agent`` factory + ``H7Agent`` OO
wrapper, following the same pattern as ABTestingAgent and
PowerAnalysisAgent.

Node type: ``source.rest_api``
"""
...
from ai_data_science_team.tools.rest_api import (  # noqa: E402, F401
...
def build_rest_connector_wrapped(config: ConnectorConfig) -> Tuple[str, dict]:
    ...
    """
    logger.info("    * Tool: h7_build_rest_connector")
    kwargs = {"config": config}
    try:
        result = build_rest_connector(**kwargs)
    except Exception as exc:
        return f"Tool h7_build_rest_connector failed: {exc}", {
            "build_rest_connector": kwargs,
            "args": kwargs,
            "result": None,
            "content": f"error: {exc}",
        }
    content = "h7_build_rest_connector: ok"
    return content, {
        "build_rest_connector": kwargs,
        "args": kwargs,
        "result": result,
...
def make_rest_api_agent(
    ...
    """Build the LangGraph StateGraph for the H7 agent."""
    ...
    def run_react_agent(state: GraphState):
        logger.info("    * RUN REACT AGENT FOR H7")
        base = state.get("messages") or [("user", state.get("user_instructions"))]
        messages = [
            (
                "system",
                "You are the H7 agent. Use the available tools to complete the user's request.",
            )
        ] + list(base)
    ...
    def post_process(state: GraphState):
        logger.info("    * POST-PROCESSING H7 RESULTS")
...
class RESTAPISourceAgent(BaseAgent):
    """OO wrapper for the H7 agent (node type ``source.rest_api``)."""
    ...
    def _make_compiled_graph(self):
        self.response = None
        return make_rest_api_agent(**self._params)
