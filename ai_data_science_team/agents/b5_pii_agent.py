...
tool layer (``ai_data_science_team.tools.b5_pii``) with
LangChain ``@tool`` decorators and exposes the standard
``make_b5_pii_agent`` factory + ``B5Agent`` OO
...
B5_TOOLS = [
    b5_scan_pii_wrapped,
    b5_default_strategies_for_wrapped,
    b5_anonymize_dataframe_wrapped,
]
...
class B5Agent(BaseAgent):
    ...
    def _make_compiled_graph(self):
        self.response = None
        return make_b5_pii_agent(**self._params)
