...
tool layer (``ai_data_science_team.tools.h5_sheets``) with
LangChain ``@tool`` decorators and exposes the standard
``make_h5_sheets_agent`` factory + ``H5Agent`` OO
...
from ai_data_science_team.tools.h5_sheets import (
...
def h5_build_sheets_connector_wrapped(config: ConnectorConfig) -> Tuple[str, {
    "name": str,
    "args": dict,
    "result": GoogleSheetsConnector,
    "content": str,
}]:
...
H5_TOOLS = [
    h5_build_sheets_connector_wrapped,
]
...
class H5Agent(BaseAgent):
    ...
    def _make_compiled_graph(self):
        self.response = None
        return make_h5_sheets_agent(**self._params)
