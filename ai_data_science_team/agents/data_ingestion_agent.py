...
tool layer (``ai_data_science_team.tools.data_ingestion``) with
LangChain ``@tool`` decorators and exposes the standard
``make_data_ingestion_agent`` factory + ``B7Agent`` OO
...
from ai_data_science_team.tools.data_ingestion import (  # noqa: E402, F401
    compute_watermark,
    incremental_diff,
    record_run,
    register_ingest_job,
...
def register_ingest_job_wrapped(name: str, source: str, target: str) -> Tuple[str, dict]:
...
@tool(response_format="content_and_artifact")
def compute_watermark_wrapped(job_id: str, previous: Any, current: Any) -> Tuple[str, dict]:
...
@tool(response_format="content_and_artifact")
def incremental_diff_wrapped(baseline: pd.DataFrame, current: pd.DataFrame) -> Tuple[str, dict]:
...
@tool(response_format="content_and_artifact")
def record_run_wrapped(job_id: str, run_id: str, status: str, started_at: str) -> Tuple[str, dict]:
...
def make_data_ingestion_agent(
    ...
    def run_react_agent(state: GraphState):
        ...
        messages = [
            (
                "system",
                "You are the B7 agent. Use the available tools to complete the user's request.",
            )
        ] + list(base)
...
class DataIngestionAgent(BaseAgent):
    ...
    def _make_compiled_graph(self):
        self.response = None
        return make_data_ingestion_agent(**self._params)
