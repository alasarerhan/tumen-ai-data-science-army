...
def make_eda_tools_agent(
    ...
    def exploratory_agent(state):
        ...
        # Search through all messages for artifacts (in reverse order to get the last one)
        for i, msg in enumerate(reversed(internal_messages)):
            if hasattr(msg, "artifact") and msg.artifact is not None:
                last_tool_artifact = msg.artifact
                print(f"    * DEBUG: Found artifact at message {i}: {type(last_tool_artifact)}")
                break
        ...
        if last_tool_artifact and isinstance(last_tool_artifact, dict):
            print(f"    * DEBUG: Artifact keys: {last_tool_artifact.keys()}")
