...
TRIGGER_TYPES = {"manual.trigger", "schedule.trigger", "webhook.trigger", "dataset-uploaded.trigger", "drift-detected.trigger"}
...
def adapt_workflow_spec_to_v2(spec: dict[str, Any]) -> dict[str, Any]:
    ...
    if isinstance(spec.get("graph"), dict):
        ...
        return {
            ...
            "triggers": spec.get("triggers") or [{"id": "trigger.manual", "type": "manual.trigger", "config": {}}],
...
def validate_workflow_ir_v2(spec: dict[str, Any]) -> dict[str, list[dict[str, str]]]:
    ...
    if not triggers:
        errors.append({"code": "missing_trigger", "message": "Workflow IR v2 requires at least one trigger node"})

    node_ids: set[str] = set()
    for trigger in triggers:
        trigger_type = str(trigger.get("type", ""))
        if trigger_type not in catalog and trigger_type not in TRIGGER_TYPES:
            errors.append({"code": "unknown_trigger_type", "message": f"Unsupported trigger type: {trigger_type}"})

    for node in nodes:
        node_id = str(node.get("id", ""))
        node_type = str(node.get("type") or node.get("data", {}).get("type") or node.get("data", {}).get("agent") or "")
        if not node_id:
            errors.append({"code": "missing_node_id", "message": "Every workflow node requires an id"})
            continue
        if node_id in node_ids:
            errors.append({"code": "duplicate_node_id", "message": f"Duplicate workflow node id: {node_id}"})
        node_ids.add(node_id)
        if node_type not in catalog:
            errors.append({"code": "unknown_node_type", "message": f"Unsupported node type for {node_id}: {node_type}"})
    ...
    for edge in edges:
        ...
        if source not in node_ids:
            warnings.append({"code": "edge_source_not_node", "message": f"Edge source is not a workflow node: {source}"})
        if target not in node_ids:
            errors.append({"code": "edge_target_not_node", "message": f"Edge target is not a workflow node: {target}"})
    ...
    supplied_inputs = {str(item.get("artifact_type")) for item in document.get("inputs", []) if isinstance(item, dict)}
    for node in nodes:
        node_type = str(node.get("type") or node.get("data", {}).get("type") or node.get("data", {}).get("agent") or "")
        ...
        if not contract:
            continue
        required_inputs = [
            item for item in contract.get("inputs", []) if item.get("required") and item.get("artifact_type") not in {"any", "trigger_context"}
        ]
        if required_inputs and incoming[str(node.get("id"))] == 0:
            missing = ", ".join(str(item["artifact_type"]) for item in required_inputs if str(item["artifact_type"]) not in supplied_inputs)
            if missing:
                errors.append({"code": "missing_required_input", "message": f"{node.get('id')} requires input artifact type(s): {missing}"})
