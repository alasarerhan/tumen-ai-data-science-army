...
tool layer (``ai_data_science_team.tools.responsible_ai``) with
LangChain ``@tool`` decorators and exposes the standard
``make_responsible_ai_agent`` factory + ``J6Agent`` OO
...
from ai_data_science_team.tools.responsible_ai import (  # noqa: E402, F401
    ErrorSlice,
    FairnessReport,
...
AGENT_NAME = "responsible_ai_agent"
...
def compute_fairness_wrapped() -> Tuple[str, dict]:
...
@tool(response_format="content_and_artifact")
def compute_explainability_wrapped() -> Tuple[str, dict]:
...
@tool(response_format="content_and_artifact")
def discover_error_slices_wrapped() -> Tuple[str, dict]:
...
@tool(response_format="content_and_artifact")
def suggest_mitigations_wrapped(
    fairness: Optional[FairnessReport], error_slices: Sequence[ErrorSlice]
) -> Tuple[str, dict]:
...
@tool(response_format="content_and_artifact")
def build_dashboard_wrapped() -> Tuple[str, dict]:
...
@tool(response_format="content_and_artifact")
def dashboard_payload_wrapped(d: ResponsibleAIDashboard) -> Tuple[str, dict]:
...
def make_responsible_ai_agent(
    ...
    def run_react_agent(state: GraphState):
        ...
        messages = [
            (
                "system",
                "You are the J6 agent. Use the available tools to complete the user's request.",
            )
        ] + list(base)
...
class ResponsibleAIAgent(BaseAgent):
    ...
    def _make_compiled_graph(self):
        self.response = None
        return make_responsible_ai_agent(**self._params)
