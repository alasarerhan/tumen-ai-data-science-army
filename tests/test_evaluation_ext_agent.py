"""
Tests for ``ai_data_science_team.agents.evaluation_ext_agent`` (F1 layer).
...
Phase-6 integration tests covering:
  ...
  3. ``make_evaluation_ext_agent`` factory returns a compiled graph with the
     expected node wiring
  4. ``EvaluationExtAgent`` (BaseAgent subclass) constructs, exposes accessors,
...
class _StubModel:
    ...
    ``make_evaluation_ext_agent`` calls ``create_agent(model, ...)`` to bind tools;
...
def _agent_with_no_op_react(monkeypatch) -> EvaluationExtAgent:
    ...
    return EvaluationExtAgent(model=_StubModel())
...
class TestModuleSurface:
    def test_constants(self):
        ...
        assert AGENT_NAME == "evaluation_ext_agent"
        assert NODE_TYPE == "model.evaluate.ext"

    def test_tool_count_matches_registry(self):
        assert len(EVALUATION_EXT_TOOLS) >= 1
    ...
    def test_tool_names_match_registry(self):
        ...
        wrapper_names = {t.name for t in EVALUATION_EXT_TOOLS}
    ...
    def test_all_individual_tools_exported(self):
        ...
        mod = sys.modules["ai_data_science_team.agents.evaluation_ext_agent"]
        wrapper_names = ["evaluate_calibration_wrapped", "optimize_threshold_wrapped", "evaluate_segments_wrapped"]
...
class TestEvaluateCalibration:
    def test_direct_call_returns_tuple(self):
        """Call evaluate_calibration via its @tool wrapper (LLM-free)."""
        from ai_data_science_team.agents.evaluation_ext_agent import evaluate_calibration_wrapped
        kwargs = {
            "y_true": [],
            "y_prob": [],
        }
        try:
            result = evaluate_calibration_wrapped.invoke(kwargs)
...
class TestOptimizeThreshold:
    def test_direct_call_returns_tuple(self):
        """Call optimize_threshold via its @tool wrapper (LLM-free)."""
        from ai_data_science_team.agents.evaluation_ext_agent import optimize_threshold_wrapped
        kwargs = {
            "y_true": [],
            "y_prob": [],
        }
        try:
            result = optimize_threshold_wrapped.invoke(kwargs)
        except Exception as exc:
            # If the placeholder kwargs are insufficient, the tool may raise.
            # We still accept the call attempt as evidence the wrapper is wired up.
            assert isinstance(exc, Exception)
...
class TestEvaluateSegments:
    def test_direct_call_returns_tuple(self):
        """Call evaluate_segments via its @tool wrapper (LLM-free)."""
        from ai_data_science_team.agents.evaluation_ext_agent import evaluate_segments_wrapped
        kwargs = {
            "df": None,
            "y_true": [],
            "y_pred": [],
        ...
        try:
            result = evaluate_segments_wrapped.invoke(kwargs)
        except Exception as exc:
            # If the placeholder kwargs are insufficient, the tool may raise.
            # We still accept the call attempt as evidence the wrapper is wired up.
            assert isinstance(exc, Exception)
...
class TestMakeEvaluationExtAgent:
    def test_factory_returns_compiled_graph(self):
        app = make_evaluation_ext_agent(model=_StubModel())
    ...
    def test_factory_with_checkpointer(self):
        ...
        app = make_evaluation_ext_agent(model=_StubModel(), checkpointer=InMemorySaver())
...
class TestEvaluationExtAgent:
    def test_init_compiles_graph(self):
        agent = EvaluationExtAgent(model=_StubModel())
