"""insight_agent modül yüzeyi + fabrika gerçek testleri (LLM-free).

Bu dosya yalnızca modülün export ettiği sabit isimleri (AGENT_NAME, NODE_TYPE,
INSIGHT_MINING_TOOLS), tool wrapper sınıflarının LangChain ``StructuredTool`` sözleşmesine
uyumluluğunu, ve ``make_insight_agent`` fabrikasının beklenen LangGraph node'larını
ürettiğini doğrular. **LLM çağrısı içermez** — bu yüzden hızlıdır ve LLM yokken
koşabilir.

Tüm tool'lar stateful (pd.DataFrame); tool davranış testleri
...
def test_constants():
    """AGENT_NAME lowercase spec_id + '_agent'; NODE_TYPE 'insight.mine'."""
...
# ---------------------------------------------------------------------------
# 3. make_insight_agent fabrika wiring (LLM-free, no monkey-patch)
# ---------------------------------------------------------------------------


class _CallableModel:
    """``make_insight_agent`` yalnızca ``model`` argümanının callability'sini
    kontrol eder; bu stub, gerçek bir LLM çağrısı yapmaz. Aşağıdaki
    ``make_insight_agent`` testleri yalnızca graf düğümlerinin varlığını kontrol eder,
...
def test_make_insight_agent_builds_graph():
    """make_insight_agent bir LangGraph StateGraph döndürmeli ve en azından
    reakt-agent + post_process node'larını içermeli."""
    graph = make_insight_agent(model=_CallableModel())
    ...
    assert len(node_ids) >= 1, f"make_insight_agent boş graph döndürdü: {node_ids}"
