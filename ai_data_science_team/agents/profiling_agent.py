...
Phase-5 agent wrapper for spec B1.  Wraps the deterministic
tool layer (``ai_data_science_team.tools.profiling``) with
LangChain ``@tool`` decorators and exposes the standard
``make_profiling_agent`` factory + ``B1Agent`` OO
...
@tool(response_format="content_and_artifact")
def profile_dataframe_wrapped(df: pd.DataFrame) -> Tuple[str, dict]:
...
def make_profiling_agent(
    ...
    """Build the LangGraph StateGraph for the B1 agent."""
...
class DataProfilingAgent(BaseAgent):
    """OO wrapper for the B1 agent (node type ``data.profile``)."""
    ...
    def _make_compiled_graph(self):
        self.response = None
        return make_profiling_agent(**self._params)
